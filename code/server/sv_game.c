...
int Q_rand() {
	return Rand_NextUInt32( &svs.db.rand );
}
...
void SV_GameSendServerCommand( int clientNum, const char *text ) {
	...
	} else if ( clientNum == -2 )
	{
		...
		for ( j=0, client = svs.clients; j < sv_maxclients->integer ; j++, client++)
		{
			...
			SV_AddServerCommand( client, (char *)text );
		}
...
void SV_LocateGameData( sharedEntity_t *gEnts, int numGEntities, int sizeofGEntity_t,
					   playerState_t *clients, int sizeofGameClient, globalState_t * gs ) {
	sv.gentities = gEnts;
...
extern int sql_global_int( const char * name );
...
intptr_t SV_GameSystemCalls( intptr_t *args ) {
	...
	case G_LOCATE_GAME_DATA:
		SV_LocateGameData( VMA(1), args[2], args[3], VMA(4), args[5], VMA(6) );
		return 0;
	case G_DROP_CLIENT:
	...
	case BOTLIB_LOAD_MAP:
		return botlib_export->BotLibLoadMap( CM_GetPreloadedData( CM_FDB_AAS ), VMA(1) );
	...
	case G_SQL_LOADDB:
		{
			...
			if ( length > 0 ) {
				sql_exec( &svs.db, buffer );
			}
			FS_FreeFile(buffer);
			
			return 0;
		} break;
	case G_SQL_EXEC:		return sql_exec( &svs.db,VMA(1) );
	case G_SQL_PREPARE:		return sql_prepare( &svs.db,VMA(1) ) != 0;
	case G_SQL_BINDTEXT:	return sql_bindtext( &svs.db,args[1], VMA(2) );
	case G_SQL_BINDINT:		return sql_bindint( &svs.db,args[1], args[2] );
	case G_SQL_STEP:		return sql_step( &svs.db );
	case G_SQL_COLUMNCOUNT:	return sql_columncount( &svs.db );
	case G_SQL_COLUMNASTEXT:
		{
			Q_strncpyz( VMA(1), sql_columnastext( &svs.db,args[3] ), args[2] );
		} break;
	case G_SQL_COLUMNASINT:	return sql_columnasint( &svs.db,args[1] );
	case G_SQL_COLUMNNAME:
		Q_strncpyz( VMA(1), sql_columnname( &svs.db,args[3] ), args[2] );
		break;
	case G_SQL_DONE:		return sql_done( &svs.db );
	case G_SQL_SELECT:		return sql_select( &svs.db, VMA(1), (char*)args[2], VMA(2), args[ 3 ] );
	case G_SQL_RESET:
		sql_reset			( &svs.db, TAG_SQL_SERVER );
		sql_assign_triggers ( &svs.db, SV_TableInsertTrigger, SV_TableUpdateTrigger, SV_TableDeleteTrigger );
		svs.db.global_int = sql_global_int;
		break;
...
int select_from_server( const char * src, char * segment, char * buffer, int size ) {
	return sql_select( &svs.db, src, segment, buffer, size );
}
...
char * SV_BootGameProgs() {
	...
	return (char *)VM_Call ( gvm, GAME_BOOT );
}
