...
==================
*/
int SV_BotAllocateClient( int player ) {
	int			i = (player)?0:MAX_PLAYERS;
	int			n = (player)?MAX_PLAYERS:MAX_CLIENTS;
	client_t	*cl;

	// find a client slot, skip the first MAX_PLAYER slots since those are reserved for players
	for ( ; i<n; i++ ) {
