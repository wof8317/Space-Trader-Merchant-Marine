...
sqlInfo_t * sql_getserverdb( void ) { return &svs.db; }
...
void QDECL SV_SendServerCommand(client_t *cl, const char *fmt, ...) {
	...
	va_start (argptr,fmt);
	Q_vsnprintf( (char*)message, sizeof( message ) - 1, fmt, argptr );
	((char*)message)[sizeof( message ) - 1] = 0;
	va_end (argptr);
...
#define	HEARTBEAT_GAME	"EMM-1"
void SV_MasterHeartbeat( void ) {
	static netadr_t	adr[MAX_MASTER_SERVERS];
	int			i;

	// com_heartbeat added instead of infering based on what dedicated was set to
	if ( com_heartbeat->integer == 0 ) {
		return;
	}
	...
	for ( i = 0 ; i < MAX_MASTER_SERVERS ; i++ ) {
		...
		//only print heartbeat messages when it's a dedicated server.
...
void SVC_Info( netadr_t from ) {
	...
	//	ignore if game is in progress
	if ( sv.gs->TURN > 1 || sv.gs->INPLAY == 1 )
		return;
	...
	count = 0;
	for ( i = sv_privateClients->integer ; i < MAX_PLAYERS ; i++ ) {
	...
	Info_SetValueForKey( infostring, "protocol", PROTOCOL_VERSION );
	Info_SetValueForKey( infostring, "hostname", va("%s's^7 Game: %s", Cvar_VariableString("name"), sv_hostname->string) );
	...
	gamedir = Cvar_VariableString( "g_missiontitle" );
	if( *gamedir ) {
...
===============
*/
void SVC_ExternalAddress( char * addr ) {
...
void SV_ConnectionlessPacket( netadr_t from, msg_t *msg ) {
	...
	} else if (!Q_stricmp(c, "port")) {
		SVC_ExternalAddress( Cmd_Argv(1) );
	} else {
