...
extern int Q_EXTERNAL_CALL CL_ScaledMilliseconds( void );
...
static void setupQuad( long xOff, long yOff )
{
	...
	if( xOff == cin.oldXOff &&
		yOff == cin.oldYOff &&
		(unsigned long)cinTable[currentHandle].ysize == (unsigned long)cin.oldysize &&
		(unsigned long)cinTable[currentHandle].xsize == (unsigned long)cin.oldxsize)
	{
		return;
	}
...
==================
*/
int Q_EXTERNAL_CALL CIN_PlayCinematic( const char *arg, int x, int y, int w, int h, int systemBits ) {
