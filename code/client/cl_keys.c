...
===================
*/
void Field_Draw( field_t *edit, float *r, char * label, float fontScale, vec4_t color )
{
	char	tmp[ MAX_EDIT_LINE + 64 ];
	int		cursor;
	const vec4_t backColor = { 0.2f, 0.2f, 0.2f, 0.75f };

	SCR_FillRect( r[ 0 ], r[ 1 ], r[ 2 ], edit->textRect[ 3 ] + 8.0f, backColor, cls.menu );

	Q_strncpyz( tmp, label, sizeof(tmp) );
	cursor = strlen( tmp ) + edit->cursor;
	Q_strcat( tmp, sizeof(tmp), edit->buffer );
...
===================
*/
void Q_EXTERNAL_CALL Key_Unbind_f( void )
{
	int		b;
...
===================
*/
void Q_EXTERNAL_CALL Key_Unbindall_f( void )
{
	int		i;
...
===================
*/
void Q_EXTERNAL_CALL Key_Bind_f (void)
{
	int			i, c, b;
	char		cmd[1024];
...
============
*/
void Q_EXTERNAL_CALL Key_Bindlist_f( void )
{
	int		i;
  ...
  if (key == K_ENTER)
  {
	...
	if ( key == K_ESCAPE && down ) {
		...
		if ( !( cls.keyCatchers & KEYCATCH_UI ) ) {
			...
			}//Scott: Disable the escape key from disconnecting you.
			/*else {
				CL_Disconnect_f();
				S_StopAllSounds();
		...
		VM_Call( uivm, UI_KEY_EVENT, 0, key, down );
		return;
	}
...
void CL_CharEvent( int key ) {
	...
	{
		VM_Call( uivm, UI_KEY_EVENT, 0, key | K_CHAR_FLAG, qtrue );
	}
	else if ( cls.keyCatchers & KEYCATCH_MESSAGE ) 
	{
