...
{
	/*
		The first few bytes of this struct are shared
		between the allocTime and the freelist.

		The only way to tell which of the two is valid seems
		to be the thesfx field. If it's null the channel is in
		the free list and allocTime is garbage. I'm not sure if
		the converse holds.
	*/
	...
	struct channel_s *nextFree;	// only valid while sfx = NULL
} channel_t;
...
typedef struct
{
	...
	void (*SoundInfo)( void );
	void (*SoundList)( void );
	char *(*DumpGetSoundString)( const char *pre, const char *post, const char *fmt );
} soundInterface_t;
