...
#else
#include "../../libs/libvorbis-1.1.2/include/vorbis/vorbisfile.h"
#endif
...
int S_OGG_Callback_seek(void *datasource, ogg_int64_t offset, int whence)
{
	...
	switch(whence)
	{
		...
		case SEEK_END :
		{
			...
			// something has gone wrong, so we return here
			// if retVal is not 0, or not the same length as the end point
			if(!(retVal == 0 || retVal == (long)stream->length + (long)offset))
			{
			 return retVal;
			}
...
int S_OGG_CodecReadStream(snd_stream_t *stream, int bytes, void *buffer)
{
	...
	while(-1)
	{
		...
		c = ov_read((OggVorbis_File *) stream->ptr, bufPtr, bytesLeft, endian, OGG_SAMPLEWIDTH, 1, &BS);
