...
void Q_EXTERNAL_CALL S_Play_f( void );
void Q_EXTERNAL_CALL S_SoundList_f( void );
void Q_EXTERNAL_CALL S_Music_f( void );
...
static snd_stream_t	*s_backgroundStream;
...
/*
=================
S_Base_DumpGetSoundString
=================
*/
char * S_Base_DumpGetSoundString( const char *pre, const char *post, const char *fmt ) {
	size_t totalLen = 1;
	char *lines;
	char *s;
	int i;
	...
	if( post )
		totalLen += strlen( post );

	for (sfx=s_knownSfx, i=0 ; i<s_numSfx ; i++, sfx++) 
	{
		char tmp[MAX_QPATH + sizeof( fmt )];
		Com_sprintf( tmp, sizeof( tmp ), fmt, sfx->soundName );

		totalLen += strlen( tmp );
	}

	lines = (char*)Z_Malloc( totalLen );
	s = lines;

	if( pre )
	{
		Com_Memcpy( s, pre, strlen( pre ) );
		s += strlen( pre );
	}
	...
	for (sfx=s_knownSfx, i=0 ; i<s_numSfx ; i++, sfx++)
	{
		...
		len = strlen( tmp );
		Com_Memcpy( s, tmp, len );
		s += len;
	}

	if( post )
	{
		Com_Memcpy( s, post, strlen( post ) );
		s += strlen( post );
	}
...
void S_Base_BeginRegistration( void ) {
	...
	if (s_numSfx == 0) {
		...
		S_Base_RegisterSound("sound/feedback/hit.ogg", qfalse);		// changed to a sound in baseq3
	}
}
...
void S_UpdateBackgroundTrack( void ) {
	...
	while ( s_rawend < s_soundtime + MAX_RAW_SAMPLES ) {
		...
		if (fileSamples < 1) fileSamples = 1;
...
//		s_numSfx = 0;
	...
	si->DumpGetSoundString = S_Base_DumpGetSoundString;
	return qtrue;
}
