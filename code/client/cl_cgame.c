...
====================
*/
void CL_GetGlconfig( vidConfig_t *glconfig ) {
...
=====================
*/
static void CL_TableSetRow( tableInfo_t * table, cellInfo_t * row, int s ) {
	...
	for ( i=0; i<table->column_count; i++ ) {
		...
		if ( c->format == INTEGER ) {
			row[ i ].integer = atoi( Cmd_Argv( s+i ) );
		} else {
			row[ i ].string = sql_alloc_string( &cl.db, Cmd_Argv( s+i ) );
		}
	}
}
...
=====================
*/
static void CL_TableUpdateRow( void ) {
	...
	if ( table && table->rows ) {
		...
		CL_TableSetRow( table, row, 3 );

		table->last_changed++;

		if ( cl.db.update_trigger ) {
			cl.db.update_trigger( &cl.db, table, i );
		}
...
=====================
*/
static void CL_TableInsertRow( void ) {

	tableInfo_t * table = find_table( &cl.db, Cmd_Argv( 1 ) );
	if ( table ) {
		cellInfo_t * row;

		row = sql_insert_begin( &cl.db, table );

		CL_TableSetRow( table, row, 2 );

		sql_insert_done( &cl.db, table );
...
{
	VM_Call( cgvm, CG_DRAW_ACTIVE_FRAME, cl.serverTime, stereo, clc.demoplaying, uiFullScreen );
	VM_Debug( 0 );
}
...
{
	VM_Call( cgvm, CG_DRAW_OVERLAYS, cl.serverTime, stereo, clc.demoplaying );
	VM_Debug( 0 );
}
...
void CL_R_LoadWorld( const char *mapName )
{
	...
	void *pMapData = CM_GetPreloadedData( CM_FDB_BSPF );
	...
	COM_StripExtension( mapName, basePath, sizeof( basePath ) );
	
	//preload all the lightmaps
	for( i = 0; ; i++ )
	{
		int len;
		void *buf;
		...
		name = va( "%s/lm_%04i.dds", basePath, i );
		len = FS_FOpenFileRead( name, &fh, qfalse );

		if( len < 0 )
			break;

		buf = Hunk_AllocateTempMemory( len );
		FS_Read( buf, len, fh );

		re.PreloadDDSImage( buf, name );
	...
	re.LoadWorldFast( pMapData, mapName );	
}
