...
void Sys_CreateConsole( void )
{
	...
	if ( g_wv.hWndHost ) {
		...
		GetClientRect( g_wv.hWndHost, &rect );
		DEDSTYLE = WS_CHILD;
		...
		sw	= GetDeviceCaps( hDC, HORZRES );
		sh	= GetDeviceCaps( hDC, VERTRES );
		ReleaseDC( GetDesktopWindow(), hDC );

		sw = (sw - (rect.right-rect.left))/2;
		sh = (sh - (rect.bottom-rect.top))/2;
