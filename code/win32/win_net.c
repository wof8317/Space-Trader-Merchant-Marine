...
void SockadrToNetadr( struct sockaddr *s, netadr_t *a, netadrtype_t type )
{
...
qboolean Sys_GetPacketData( netadr_t *net_from, msg_t *net_message)
{
 	int 	ret;
	...
	ret = recvfrom( net_from->socket, net_message->data, net_message->maxsize, 0, (struct sockaddr *)&from, &fromlen );
	if (ret == SOCKET_ERROR)
	{
		err = WSAGetLastError();
	...
	if ( net_from->socket == ip_socket ) {
		memset( ((struct sockaddr_in *)&from)->sin_zero, 0, 8 );
	}

	if ( usingSocks && net_from->socket == ip_socket && memcmp( &from, &socksRelayAddr, fromlen ) == 0 ) {
		...
		SockadrToNetadr( &from, net_from, net_from->type );
...
qboolean Sys_GetPacket( netadr_t *net_from, msg_t *net_message ) {
	...
	for( protocol = 0 ; protocol < 2 ; protocol++ )	{
		if( protocol == 0 ) {
			net_from->socket = ip_socket;
		}
		else {
			net_from->socket = ipx_socket;
		}

		if( !net_from->socket ) {
			continue;
		}
		if (Sys_GetPacketData( net_from, net_message) == qtrue)
			return qtrue;
	}
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	else if ( to.type == NA_TCP ) {
		net_socket = to.socket;
	}
	else {
...
====================
*/
int NET_IPSocket( char *net_interface, int port, netadrtype_t type ) {
	SOCKET				newsocket;
	...
	if (type == NA_IP)
	{
		...
		if( ( newsocket = socket( AF_INET, SOCK_STREAM, IPPROTO_TCP ) ) == INVALID_SOCKET ) {
			err = WSAGetLastError();
			if( err != WSAEAFNOSUPPORT ) {
				Com_Printf( "WARNING: TCP_OpenSocket: socket: %s\n", NET_ErrorString() );
			}
			return 0;
		}
	}
	...
	if( ioctlsocket( newsocket, FIONBIO, (u_long *)&_true ) == SOCKET_ERROR ) {
		Com_Printf( "WARNING: OpenSocket: ioctl FIONBIO: %s\n", NET_ErrorString() );
		return 0;
	}
	...
	{
		int resp;
		resp = connect( newsocket, (struct sockaddr *)&address, sizeof(address));
		if ( resp == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK )
		{
			Com_Printf( "WARNING: TCP_OpenSocket: connect: %s\n", NET_ErrorString() );
			closesocket( newsocket );
			return 0;
		}
	}
...
void NET_OpenIP( void ) {
	...
	for( i = 0 ; i < 10 ; i++ ) {
		ip_socket = NET_IPSocket( ip->string, port + i, NA_IP );
		if ( ip_socket ) {
