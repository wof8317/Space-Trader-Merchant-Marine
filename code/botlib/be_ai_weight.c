								...
								fuzzyseperator_t *fsout)
{
	if (fs1->child)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "cannot interbreed weight configs, unequal child\n");
			return qfalse;
		} //end if
	...
	else if (fs1->type == WT_BALANCE)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "cannot interbreed weight configs, unequal balance\n");
			return qfalse;
		} //end if
	...
	if (fs1->next)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "cannot interbreed weight configs, unequal next\n");
			return qfalse;
		} //end if
