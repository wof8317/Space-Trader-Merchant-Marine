...
int AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	...
	if (trace.startsolid)
	{
		BOTPRINT(PRT_MESSAGE, "trigger_push start solid\n");
	...
	if (!ent2)
	{
		BOTPRINT(PRT_MESSAGE, "trigger_push without target entity %s\n", target);
		return qfalse;
	} //end if
	...
	if (!time)
	{
		BOTPRINT(PRT_MESSAGE, "trigger_push without time\n");
		return qfalse;
	} //end if
...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!link)
		{
			BOTPRINT(PRT_MESSAGE, "trigger_push not in any jump pad area\n");
			AAS_UnlinkFromAreas(areas);
			continue;
		} //end if
		//
		//BOTPRINT(PRT_MESSAGE, "found a trigger_push with velocity %f %f %f\n", velocity[0], velocity[1], velocity[2]);
		//
...
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs, vec3_t goalorigin)
{
	...
	if (!aasworld.loaded)
	{
		BOTPRINT(PRT_ERROR, "AAS_BestReachableArea: aas not loaded\n");
		return 0;
	} //end if
	...
	if (areanum)
	{
		...
		else
		{
			...
			BOTPRINT(PRT_MESSAGE, "AAS_BestReachableArea: start solid\n");
#endif
...
void AAS_Reachability_Teleport(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!strcmp(classname, "trigger_multiple"))
		{
			AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY);
//#ifdef REACH_DEBUG
			BOTPRINT(PRT_MESSAGE, "trigger_multiple model = \"%s\"\n", model);
//#endif REACH_DEBUG
			VectorClear(angles);
			...
			{
				BOTPRINT(PRT_ERROR, "trigger_multiple at %1.0f %1.0f %1.0f without target\n",
			...
			{
				BOTPRINT(PRT_ERROR, "target_teleporter without target\n");
				continue;
			} //end if
		} //end else
		else if (!strcmp(classname, "trigger_teleport"))
		{
			AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY);
//#ifdef REACH_DEBUG
			BOTPRINT(PRT_MESSAGE, "trigger_teleport model = \"%s\"\n", model);
//#endif REACH_DEBUG
			VectorClear(angles);
			AAS_BSPModelMinsMaxsOrigin(atoi(model+1), angles, mins, maxs, origin);
			//
			if (!AAS_ValueForBSPEpairKey(ent, "target", target, MAX_EPAIRKEY))
			{
				BOTPRINT(PRT_ERROR, "trigger_teleport at %1.0f %1.0f %1.0f without target\n",
									origin[0], origin[1], origin[2]);
				continue;
			} //end if
		} //end if
		...
		if (!dest)
		{
			BOTPRINT(PRT_ERROR, "teleporter without misc_teleporter_dest (%s)\n", target);
			continue;
		} //end if
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num))
		{
			...
			else*/
			{
				...
				if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA))
				{
					BOTPRINT(PRT_WARNING, "teleported into slime or lava at dest %s\n", target);
				} //end if
				VectorCopy(move.endpos, destorigin);
			} //end else
		} //end if
		//
		//BOTPRINT(PRT_MESSAGE, "teleporter brush origin at %f %f %f\n", origin[0], origin[1], origin[2]);
		//BOTPRINT(PRT_MESSAGE, "teleporter brush mins = %f %f %f\n", mins[0], mins[1], mins[2]);
		//BOTPRINT(PRT_MESSAGE, "teleporter brush maxs = %f %f %f\n", maxs[0], maxs[1], maxs[2]);
		...
		if (!areas) BOTPRINT(PRT_MESSAGE, "trigger_multiple not in any area\n");
		//
		for (link = areas; link; link = link->next_area)
		{
...
void AAS_Reachability_Elevator(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!strcmp(classname, "func_plat"))
		{
			...
			{
				BOTPRINT(PRT_ERROR, "func_plat without model\n");
				continue;
			} //end if
			...
			if (modelnum <= 0)
			{
				BOTPRINT(PRT_ERROR, "func_plat with invalid model number\n");
				continue;
			} //end if
			//get the mins, maxs and origin of the model
			...
			//
			//BOTPRINT(PRT_MESSAGE, "platbottom[2] = %1.1f plattop[2] = %1.1f\n", platbottom[2], plattop[2]);
			//
			VectorAdd(mins, maxs, mids);
...
void AAS_Reachability_FuncBobbing(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY))
		{
			BOTPRINT(PRT_ERROR, "func_bobbing without model\n");
			continue;
		} //end if
		//get the model number, and skip the leading *
		modelnum = atoi(model+1);
		if (modelnum <= 0)
		{
			BOTPRINT(PRT_ERROR, "func_bobbing with invalid model number\n");
			continue;
		} //end if
...
//		VectorScale(velocity, speed, velocity);
		...
		//
#ifdef REACH_DEBUG
		BOTPRINT(PRT_MESSAGE, "absmins = %f %f %f\n", absmins[0], absmins[1], absmins[2]);
		BOTPRINT(PRT_MESSAGE, "absmaxs = %f %f %f\n", absmaxs[0], absmaxs[1], absmaxs[2]);
#endif REACH_DEBUG
		VectorAdd(absmins, absmaxs, origin);
		VectorScale (origin, 0.5, origin);
...
void AAS_SetWeaponJumpAreaFlags(void)
{
			...
			!strcmp(classname, "item_invulnerability"))
		{
			if (AAS_VectorForBSPEpairKey(ent, "origin", origin))
			{
				...
				if (!(spawnflags & 1))
				{
					...
					{
						BOTPRINT(PRT_MESSAGE, "%s in solid at (%1.1f %1.1f %1.1f)\n",
				...
				//	BOTPRINT(PRT_MESSAGE, "area not grounded\n");
				//
				weaponjumpareas++;
			} //end if
		} //end if
	...
	BOTPRINT(PRT_MESSAGE, "%d weapon jump areas\n", weaponjumpareas);
...
int AAS_ContinueInitReachability(float time)
{
	...
	{
		BOTPRINT(PRT_MESSAGE, "calculating reachability...\n");
		lastpercentage = 0;
	...
	{
		BOTPRINT(PRT_MESSAGE, "\r%6.1f%%", (float) 100.0);
	...
	else if (aasworld.numreachabilityareas == aasworld.numareas + 1)
	{
		...
		//
#ifdef _DEBUG
		BOTPRINT(PRT_MESSAGE, "%6d reach swim\n", reach_swim);
		BOTPRINT(PRT_MESSAGE, "%6d reach equal floor\n", reach_equalfloor);
		BOTPRINT(PRT_MESSAGE, "%6d reach step\n", reach_step);
		BOTPRINT(PRT_MESSAGE, "%6d reach barrier\n", reach_barrier);
		BOTPRINT(PRT_MESSAGE, "%6d reach waterjump\n", reach_waterjump);
		BOTPRINT(PRT_MESSAGE, "%6d reach walkoffledge\n", reach_walkoffledge);
		BOTPRINT(PRT_MESSAGE, "%6d reach jump\n", reach_jump);
		BOTPRINT(PRT_MESSAGE, "%6d reach ladder\n", reach_ladder);
		BOTPRINT(PRT_MESSAGE, "%6d reach walk\n", reach_walk);
		BOTPRINT(PRT_MESSAGE, "%6d reach teleport\n", reach_teleport);
		BOTPRINT(PRT_MESSAGE, "%6d reach funcbob\n", reach_funcbob);
		BOTPRINT(PRT_MESSAGE, "%6d reach elevator\n", reach_elevator);
		BOTPRINT(PRT_MESSAGE, "%6d reach grapple\n", reach_grapple);
		BOTPRINT(PRT_MESSAGE, "%6d reach rocketjump\n", reach_rocketjump);
		BOTPRINT(PRT_MESSAGE, "%6d reach jumppad\n", reach_jumppad);
#endif
		//*/
		...
		aasworld.numreachabilityareas++;
		//
		BOTPRINT(PRT_MESSAGE, "calculating clusters...\n");
	} //end if
	else
	{
		lastpercentage = aasworld.numreachabilityareas * 1000 / aasworld.numareas;
		BOTPRINT(PRT_MESSAGE, "\r%6.1f%%", (float) lastpercentage / 10);
	} //end else
	//not yet finished
	return qtrue;
