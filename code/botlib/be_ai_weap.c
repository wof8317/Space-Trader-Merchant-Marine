...
int BotValidWeaponNumber(int weaponnum)
{
	...
	{
		BOTPRINT(PRT_ERROR, "weapon number out of range\n");
		return qfalse;
	} //end if
	return qtrue;
...
weaponconfig_t *LoadWeaponConfig(char *filename)
{
	...
	if (max_weaponinfo < 0)
	{
		BOTPRINT(PRT_ERROR, "max_weaponinfo = %d\n", max_weaponinfo);
		max_weaponinfo = 32;
	...
	{
		BOTPRINT(PRT_ERROR, "max_projectileinfo = %d\n", max_projectileinfo);
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "weaponinfo"))
		{
			...
			{
				BOTPRINT(PRT_ERROR, "weapon info number %d out of range in %s\n", weaponinfo.number, path);
				FreeMemory(wc);
				FreeSource(source);
				return NULL;
			} //end if
		...
		else if (!strcmp(token.string, "projectileinfo"))
		{
			...
			{
				BOTPRINT(PRT_ERROR, "more than %d projectiles defined in %s\n", max_projectileinfo, path);
				FreeMemory(wc);
				FreeSource(source);
				return NULL;
			} //end if
		...
		{
			BOTPRINT(PRT_ERROR, "unknown definition %s in %s\n", token.string, path);
			FreeMemory(wc);
			FreeSource(source);
			return NULL;
		} //end else
	...
	for (i = 0; i < wc->numweapons; i++)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "weapon %d has no name in %s\n", i, path);
			FreeMemory(wc);
			return NULL;
		} //end if
		if (!wc->weaponinfo[i].projectile[0])
		{
			BOTPRINT(PRT_ERROR, "weapon %s has no projectile in %s\n", wc->weaponinfo[i].name, path);
			FreeMemory(wc);
			return NULL;
		} //end if
		...
		{
			BOTPRINT(PRT_ERROR, "weapon %s uses undefined projectile in %s\n", wc->weaponinfo[i].name, path);
			FreeMemory(wc);
			return NULL;
		} //end if
	} //end for
	if (!wc->numweapons) BOTPRINT(PRT_WARNING, "no weapon info loaded\n");
...
int BotLoadWeaponWeights(int weaponstate, char *filename)
{
	...
	{
		BOTPRINT(PRT_FATAL, "couldn't load weapon config %s\n", filename);
...
int BotSetupWeaponAI(void)
{
	...
	if (!weaponconfig)
	{
		BOTPRINT(PRT_FATAL, "couldn't load the weapon config\n");
