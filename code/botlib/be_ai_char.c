...
bot_character_t *BotCharacterFromHandle(int handle)
{
	...
	{
		BOTPRINT(PRT_FATAL, "character handle %d out of range\n", handle);
		return NULL;
	} //end if
	if (!botcharacters[handle])
	{
		BOTPRINT(PRT_FATAL, "invalid character %d\n", handle);
		return NULL;
	} //end if
...
int BotLoadCachedCharacter(char *charfile, float skill, int reload)
{
	...
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			BOTPRINT(PRT_MESSAGE, "loaded cached skill %f from %s\n", skill, charfile);
			return cachedhandle;
		} //end if
	} //end else
	//
	...
	if (ch)
	{
		...
		//
		BOTPRINT(PRT_MESSAGE, "loaded skill %d from %s\n", intskill, charfile);
#ifdef _DEBUG
		if (bot_developer)
		{
			BOTPRINT(PRT_MESSAGE, "skill %d loaded in %d msec from %s\n", intskill, Sys_MilliSeconds() - starttime, charfile);
		} //end if
#endif //_DEBUG
		return handle;
	} //end if
	//
	BOTPRINT(PRT_WARNING, "couldn't find skill %d in %s\n", intskill, charfile);
	//
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			BOTPRINT(PRT_MESSAGE, "loaded cached default skill %d from %s\n", intskill, charfile);
			return cachedhandle;
		} //end if
	} //end if
	...
	{
		botcharacters[handle] = ch;
		BOTPRINT(PRT_MESSAGE, "loaded default skill %d from %s\n", intskill, charfile);
		return handle;
	} //end if
	//
	if (!reload)
	...
	{
		botcharacters[handle] = ch;
		BOTPRINT(PRT_MESSAGE, "loaded skill %f from %s\n", ch->skill, charfile);
		return handle;
	} //end if
	//
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			BOTPRINT(PRT_MESSAGE, "loaded cached default skill %f from %s\n", botcharacters[cachedhandle]->skill, charfile);
			return cachedhandle;
		} //end if
	} //end if
	//try to load a character with any skill
	...
	{
		botcharacters[handle] = ch;
		BOTPRINT(PRT_MESSAGE, "loaded default skill %f from %s\n", ch->skill, charfile);
		return handle;
	} //end if
	//
	BOTPRINT(PRT_WARNING, "couldn't load any skill from %s\n", charfile);
...
int CheckCharacteristicIndex(int character, int index)
{
	...
	if (index < 0 || index >= MAX_CHARACTERISTICS)
	{
		BOTPRINT(PRT_ERROR, "characteristic %d does not exist\n", index);
		return qfalse;
	} //end if
	if (!ch->c[index].type)
	{
		BOTPRINT(PRT_ERROR, "characteristic %d is not initialized\n", index);
		return qfalse;
	} //end if
	return qtrue;
...
float Characteristic_Float(int character, int index)
{
	...
	{
		BOTPRINT(PRT_ERROR, "characteristic %d is not a float\n", index);
		return 0;
	} //end else if
//	return 0;
...
float Characteristic_BFloat(int character, int index, float min, float max)
{
	...
	if (min > max)
	{
		BOTPRINT(PRT_ERROR, "cannot bound characteristic %d between %f and %f\n", index, min, max);
		return 0;
	} //end if
...
int Characteristic_Integer(int character, int index)
{
	...
	{
		BOTPRINT(PRT_ERROR, "characteristic %d is not a integer\n", index);
		return 0;
	} //end else if
//	return 0;
...
int Characteristic_BInteger(int character, int index, int min, int max)
{
	...
	if (min > max)
	{
		BOTPRINT(PRT_ERROR, "cannot bound characteristic %d between %d and %d\n", index, min, max);
		return 0;
	} //end if
...
void Characteristic_String(int character, int index, char *buf, int size)
{
	...
	{
		BOTPRINT(PRT_ERROR, "characteristic %d is not a string\n", index);
		return;
	} //end else if
	return;
