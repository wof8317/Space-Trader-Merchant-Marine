...
{
	BOTPRINT(PRT_MESSAGE, "%d area cache updates\n", numareacacheupdates);
	BOTPRINT(PRT_MESSAGE, "%d portal cache updates\n", numportalcacheupdates);
	BOTPRINT(PRT_MESSAGE, "%d bytes routing cache\n", routingcachesize);
...
/*#ifdef ROUTING_DEBUG
		...
		{
			BOTPRINT(PRT_ERROR, "portal %d: does not belong to cluster %d\n"
											, -areacluster, cluster);
		} //end if
#endif //ROUTING_DEBUG*/
...
int AAS_EnableRoutingArea(int areanum, int enable)
{
	...
	if (areanum <= 0 || areanum >= aasworld.numareas)
	{
		if (bot_developer)
		{
			BOTPRINT(PRT_ERROR, "AAS_EnableRoutingArea: areanum %d out of range\n", areanum);
		} //end if
		return 0;
	} //end if
...
void AAS_CreateReversedReachability(void)
{
	...
	for (i = 1; i < aasworld.numareas; i++)
	{
		...
		if (settings->numreachableareas >= 128)
			BOTPRINT(PRT_WARNING, "area %d has more than 128 reachabilities\n", i);
...
#ifdef _DEBUG
	BOTPRINT(PRT_MESSAGE, "reversed reachability %d msec\n", Sys_MilliSeconds() - starttime);
#endif
...
#ifdef _DEBUG
	BOTPRINT(PRT_MESSAGE, "area travel times %d msec\n", Sys_MilliSeconds() - starttime);
#endif
...
void AAS_InitPortalMaxTravelTimes(void)
{
	...
	for (i = 0; i < aasworld.numportals; i++)
	{
		...
		//BOTPRINT(PRT_MESSAGE, "portal %d max tt = %d\n", i, aasworld.portalmaxtraveltimes[i]);
	} //end for
...
void AAS_CreateAllRoutingCache(void)
{
	...
	BOTPRINT(PRT_MESSAGE, "AAS_CreateAllRoutingCache\n");
	for (i = 1; i < aasworld.numareas; i++)
	{
...
void AAS_WriteRouteCache(void)
{
	...
	*/
	...
	BOTPRINT(PRT_MESSAGE, "written %d bytes of routing cache\n", totalsize);
...
int AAS_AreaRouteToGoalArea(int areanum, vec3_t origin, int goalareanum, int travelflags, int *traveltime, int *reachnum)
{
	...
	if (areanum <= 0 || areanum >= aasworld.numareas)
	{
		if (bot_developer)
		{
			BOTPRINT(PRT_ERROR, "AAS_AreaTravelTimeToGoalArea: areanum %d out of range\n", areanum);
		} //end if
		return qfalse;
	} //end if
	if (goalareanum <= 0 || goalareanum >= aasworld.numareas)
	{
		if (bot_developer)
		{
			BOTPRINT(PRT_ERROR, "AAS_AreaTravelTimeToGoalArea: goalareanum %d out of range\n", goalareanum);
		} //end if
		return qfalse;
	} //end if
							...
							int stopevent, int stopcontents, int stoptfl, int stopareanum)
{
	...
	for (i = 0; curareanum != goalareanum && (!maxareas || i < maxareas) && i < aasworld.numareas; i++)
	{
		...
		if (reachnum > aasworld.reachabilitysize || reachnum < 0) {
			Com_Error( ERR_DROP, "AAS_PredictRoute: reachnum[%d] out of range [%d]\n", reachnum, aasworld.reachabilitysize);
		}
...
int AAS_NextAreaReachability(int areanum, int reachnum)
{
	...
	if (areanum <= 0 || areanum >= aasworld.numareas)
	{
		BOTPRINT(PRT_ERROR, "AAS_NextAreaReachability: areanum %d out of range\n", areanum);
		return 0;
	} //end if
	...
	{
		BOTPRINT(PRT_FATAL, "AAS_NextAreaReachability: reachnum < settings->firstreachableara");
		return 0;
	} //end if
	reachnum++;
			...
			if (t > 0)
			{
				if (AAS_AreaSwim(n))
				{
					*goalareanum = n;
					...
					//BOTPRINT(PRT_MESSAGE, "found random goal area %d\n", *goalareanum);
					return qtrue;
				} //end if
