...
qboolean ValidClientNumber(int num, char *str)
{
	if (num < 0 || num > botlibglobals.maxclients)
	{
		...
		BOTPRINT(PRT_ERROR, "%s: invalid client number %d, [0, %d]\n",
...
qboolean ValidEntityNumber(int num, char *str)
{
	...
	{
		BOTPRINT(PRT_ERROR, "%s: invalid entity number %d, [0, %d]\n",
...
qboolean BotLibSetup(char *str)
{
	...
	{
		BOTPRINT(PRT_ERROR, "%s: bot library used before being setup\n", str);
		return qfalse;
	} //end if
	return qtrue;
...
#endif
	//
	BOTPRINT(PRT_MESSAGE, "------- BotLib Initialization -------\n");
	//
...
// Changes Globals:		-
//===========================================================================
int Export_BotLibLoadMap( void *pAasData, const char *mapname )
{
#ifdef _DEBUG
	...
	//
	BOTPRINT(PRT_MESSAGE, "------------ Map Loading ------------\n");
	//startup AAS for the current map, model and sound index
	errnum = AAS_LoadMap( pAasData, mapname );
	if (errnum != BLERR_NOERROR) return errnum;
...
#ifdef _DEBUG
	BOTPRINT(PRT_MESSAGE, "map loaded in %d msec\n", Sys_MilliSeconds() - starttime);
#endif
	//
	return BLERR_NOERROR;
...
//	bot_goal_t goal;
	...
	//Scott: commented out as they were unused.
...
//	AAS_ClearShownDebugLines();
	...
	//if (AAS_AgainstLadder(parm2)) BOTPRINT(PRT_MESSAGE, "against ladder\n");
	//BotOnGround(parm2, PRESENCE_NORMAL, 1, &newarea, &newarea);
	//BOTPRINT(PRT_MESSAGE, "%f %f %f\n", parm2[0], parm2[1], parm2[2]);
	//*
	...
	BOTPRINT(PRT_MESSAGE, "\rtravel time to goal (%d) = %d  ", botlibglobals.goalareanum,
	...
	if (newarea != area)
	{
		...
		BOTPRINT(PRT_MESSAGE, "num areas = %d, area = %d\n", numareas, areas[0]);
		*/
		/*
		botlibglobals.goalareanum = newarea;
		VectorCopy(parm2, botlibglobals.goalorigin);
		BOTPRINT(PRT_MESSAGE, "new goal %2.1f %2.1f %2.1f area %d\n",
...
//		if (botlibglobals.runai) BOTPRINT(PRT_MESSAGE, "started AI\n");
//		else BOTPRINT(PRT_MESSAGE, "stopped AI\n");
		//* /
		/*
...
//		BOTPRINT(PRT_MESSAGE, "travel time to goal = %d\n",
//					AAS_AreaTravelTimeToGoalArea(area, origin, botlibglobals.goalareanum, TFL_DEFAULT));
//		BOTPRINT(PRT_MESSAGE, "test rj from 703 to 716\n");
...
//		AAS_TraceClientBBox(eye, end, PRESENCE_NORMAL, 1);
	...
	end_time = clock();
	BOTPRINT(PRT_MESSAGE, "me %lu clocks, %lu CLOCKS_PER_SEC\n", end_time - start_time, CLOCKS_PER_SEC);
	start_time = clock();
	for (i = 0; i < 2000; i++)
	{
	...
	end_time = clock();
	BOTPRINT(PRT_MESSAGE, "id %lu clocks, %lu CLOCKS_PER_SEC\n", end_time - start_time, CLOCKS_PER_SEC);
*/
...
botlib_export_t *GetBotLibAPI(int apiVersion, botlib_import_t *import) {
  ...
  assert(botimport.Print);   // bk001129 - pars pro toto
	...
	if ( apiVersion != BOTLIB_API_VERSION ) {
		BOTPRINT( PRT_ERROR, "Mismatched BOTLIB_API_VERSION: expected %i, got %i\n", BOTLIB_API_VERSION, apiVersion );
		return NULL;
	}
