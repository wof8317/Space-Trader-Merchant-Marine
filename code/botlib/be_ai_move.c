...
void BotFreeMoveState(int handle)
{
	...
	{
		BOTPRINT(PRT_FATAL, "move state handle %d out of range\n", handle);
		return;
	} //end if
	if (!botmovestates[handle])
	{
		BOTPRINT(PRT_FATAL, "invalid move state %d\n", handle);
		return;
	} //end if
...
//#ifdef _DEBUG
	//BOTPRINT(PRT_MESSAGE, "no reachability area\n");
//#endif //_DEBUG
	return firstareanum;
} //end of the function BotReachabilityArea*/
//===========================================================================
...
int BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach)
{
	...
	{
		BOTPRINT(PRT_MESSAGE, "no entity with model %d\n", modelnum);
		return qfalse;
	} //end if
	//
	for (i = 0; i < 2; i++)
...
void BotSetBrushModelTypes(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		{
			BOTPRINT(PRT_MESSAGE, "entity %s model number out of range\n", classname);
			continue;
		} //end if
									  ...
									  struct bot_avoidspot_s *avoidspots, int numavoidspots, int *flags)
{
		...
		reachnum = AAS_NextAreaReachability(areanum, reachnum))
	{
		...
		if (i != MAX_AVOIDREACH && avoidreachtries[i] > AVOIDREACH_TRIES)
		{
			...
			if (bot_developer)
			{
				BOTPRINT(PRT_MESSAGE, "avoiding reachability %d\n", avoidreach[i]);
			} //end if
#endif //_DEBUG
			continue;
		} //end if
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		if (type & MOVE_JUMP)
		{
			//BOTPRINT(PRT_MESSAGE, "trying jump\n");
			cmdmove[2] = 400;
		...
		{
			//BOTPRINT(PRT_MESSAGE, "client %d: max prediction frames\n", ms->client);
			return qfalse;
		} //end if
		//don't enter slime or lava and don't fall from too high
		if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE))
		{
			//BOTPRINT(PRT_MESSAGE, "client %d: would be hurt ", ms->client);
			//if (move.stopevent & SE_ENTERSLIME) BOTPRINT(PRT_MESSAGE, "slime\n");
			//if (move.stopevent & SE_ENTERLAVA) BOTPRINT(PRT_MESSAGE, "lava\n");
			//if (move.stopevent & SE_HITGROUNDDAMAGE) BOTPRINT(PRT_MESSAGE, "hitground\n");
			return qfalse;
		} //end if
		//if ground was hit
		...
		if (ms->moveflags & MFL_WALK && speed > 200.0f) {
			speed = 200.0f;
		}
		EA_Move(ms->client, hordir, speed);
...
#ifdef _DEBUG
		//BOTPRINT(PRT_MESSAGE, "%d: BotCheckBlocked: I'm blocked\n", ms->client);
#endif //_DEBUG
	} //end if
...
bot_moveresult_t BotFinishTravel_Walk(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	{
#ifdef _DEBUG
		BOTPRINT(PRT_MESSAGE, "BotFinishTravel_Walk: already in reach area\n");
#endif //_DEBUG
		return result;
	} //end if*/
...
bot_moveresult_t BotTravel_WaterJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	hordir[2] = 0;
	dir[2] += 15 + crandom() * 40;
	//BOTPRINT(PRT_MESSAGE, "BotTravel_WaterJump: dir[2] = %f\n", dir[2]);
	VectorNormalize(dir);
	dist = VectorNormalize(hordir);
	//elemantary actions
	//EA_Move(ms->client, dir, 400);
...
bot_moveresult_t BotFinishTravel_WaterJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	//BOTPRINT(PRT_MESSAGE, "BotFinishTravel_WaterJump\n");
...
//	if (dist1 && dist1 <= trace.fraction * 80) VectorMA(reach->start, dist1-20, hordir, trace.endpos);
	...
	{
		//BOTPRINT(PRT_MESSAGE, "between jump start and run to point\n");
		hordir[0] = reach->end[0] - ms->origin[0];
		hordir[1] = reach->end[1] - ms->origin[1];
		hordir[2] = 0;
		VectorNormalize(hordir);
	...
	{
		//BOTPRINT(PRT_MESSAGE, "going towards run to point\n");
...
//	vec3_t runstart, dir1, dir2, hordir;
	...
	if (DotProduct(dir1, dir2) < -0.8 || dist2 < 5)
	{
//		BOTPRINT(PRT_MESSAGE, "between jump start and run start point\n");
		hordir[0] = reach->end[0] - ms->origin[0];
		hordir[1] = reach->end[1] - ms->origin[1];
		hordir[2] = 0;
		VectorNormalize(hordir);
	...
	{
//		BOTPRINT(PRT_MESSAGE, "going towards run start point\n");
...
//	if ((ms->moveflags & MFL_AGAINSTLADDER))
		...
		// || !(ms->moveflags & MFL_ONGROUND))
	{
		//BOTPRINT(PRT_MESSAGE, "against ladder or not on ground\n");
		VectorSubtract(reach->end, ms->origin, dir);
		VectorNormalize(dir);
...
/*	else
	{
		//BOTPRINT(PRT_MESSAGE, "moving towards ladder\n");
		VectorSubtract(reach->end, ms->origin, dir);
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	if (BotOnMover(ms->origin, ms->entitynum, reach))
	{
		...
		BOTPRINT(PRT_MESSAGE, "bot on elevator\n");
#endif //DEBUG_ELEVATOR
		//if vertically not too far from the end point
		if (abs(ms->origin[2] - reach->end[2]) < sv_maxbarrier->value)
		{
#ifdef DEBUG_ELEVATOR
			BOTPRINT(PRT_MESSAGE, "bot moving to end\n");
#endif //DEBUG_ELEVATOR
		...
		else
		{
			...
			{
#ifdef DEBUG_ELEVATOR
				BOTPRINT(PRT_MESSAGE, "bot moving to center\n");
#endif //DEBUG_ELEVATOR
	...
	{
#ifdef DEBUG_ELEVATOR
		BOTPRINT(PRT_MESSAGE, "bot not on elevator\n");
#endif //DEBUG_ELEVATOR
		...
		{
#ifdef DEBUG_ELEVATOR
			BOTPRINT(PRT_MESSAGE, "elevator not down\n");
#endif //DEBUG_ELEVATOR
			dist = dist1;
			VectorCopy(dir1, dir);
			//
		...
		{
#ifdef DEBUG_ELEVATOR
			BOTPRINT(PRT_MESSAGE, "bot moving to start\n");
#endif //DEBUG_ELEVATOR
			dist = dist1;
			VectorCopy(dir1, dir);
		} //end else
...
void BotFuncBobStartEnd(aas_reachability_t *reach, vec3_t start, vec3_t end, vec3_t origin)
{
	...
	if (!AAS_OriginOfMoverWithModelNum(modelnum, origin))
	{
		BOTPRINT(PRT_MESSAGE, "BotFuncBobStartEnd: no entity with model %d\n", modelnum);
...
bot_moveresult_t BotTravel_FuncBobbing(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	{
#ifdef DEBUG_FUNCBOB
		BOTPRINT(PRT_MESSAGE, "bot on func_bobbing\n");
#endif
		...
		{
#ifdef DEBUG_FUNCBOB
			BOTPRINT(PRT_MESSAGE, "bot moving to reachability end\n");
#endif
			//move to the end point
			VectorSubtract(reach->end, ms->origin, hordir);
			hordir[2] = 0;
		...
		else
		{
			...
			{
#ifdef DEBUG_FUNCBOB
				BOTPRINT(PRT_MESSAGE, "bot moving to func_bobbing center\n");
#endif
				//move to the center of the plat
				if (dist > 100) dist = 100;
				speed = 400 - (400 - 4 * dist);
	...
	{
#ifdef DEBUG_FUNCBOB
		BOTPRINT(PRT_MESSAGE, "bot not ontop of func_bobbing\n");
#endif
		//if very near the reachability end
		VectorSubtract(reach->end, ms->origin, dir);
		dist = VectorLength(dir);
		...
		{
#ifdef DEBUG_FUNCBOB
			BOTPRINT(PRT_MESSAGE, "func_bobbing not at start\n");
#endif
			dist = dist1;
			VectorCopy(dir1, dir);
			//
		...
		{
#ifdef DEBUG_FUNCBOB
			BOTPRINT(PRT_MESSAGE, "bot moving to reachability start\n");
#endif
			dist = dist1;
			VectorCopy(dir1, dir);
		} //end else
...
#ifdef DEBUG_GRAPPLE
			BOTPRINT(PRT_MESSAGE, "reset grapple\n");
#endif //DEBUG_GRAPPLE
		} //end if
	} //end if
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	{
#ifdef DEBUG_GRAPPLE
		BOTPRINT(PRT_MESSAGE, "BotTravel_Grapple: active grapple\n");
#endif //DEBUG_GRAPPLE
		//
		...
		if (state && dist < 48)
		{
			...
			{
#ifdef DEBUG_GRAPPLE
				BOTPRINT(PRT_ERROR, "grapple normal end\n");
#endif //DEBUG_GRAPPLE
				if (offhandgrapple->value)
					EA_Command(ms->client, cmd_grappleoff->string);
				ms->moveflags &= ~MFL_ACTIVEGRAPPLE;
		...
		else if (!state || (state == 2 && dist > ms->lastgrappledist - 2))
		{
			...
			{
#ifdef DEBUG_GRAPPLE
				BOTPRINT(PRT_ERROR, "grapple not visible\n");
#endif //DEBUG_GRAPPLE
				if (offhandgrapple->value)
					EA_Command(ms->client, cmd_grappleoff->string);
				ms->moveflags &= ~MFL_ACTIVEGRAPPLE;
	...
	{
#ifdef DEBUG_GRAPPLE
		BOTPRINT(PRT_MESSAGE, "BotTravel_Grapple: inactive grapple\n");
#endif //DEBUG_GRAPPLE
		//
		ms->grapplevisible_time = AAS_Time();
		//
		...
		{
#ifdef DEBUG_GRAPPLE
			BOTPRINT(PRT_MESSAGE, "BotTravel_Grapple: activating grapple\n");
#endif //DEBUG_GRAPPLE
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	//BOTPRINT(PRT_MESSAGE, "BotTravel_RocketJump: bah\n");
	//
	hordir[0] = reach->start[0] - ms->origin[0];
	hordir[1] = reach->start[1] - ms->origin[1];
	hordir[2] = 0;
...
int BotReachabilityTime(aas_reachability_t *reach)
{
	switch(reach->traveltype & TRAVELTYPE_MASK)
	{
		...
		default:
		{
			BOTPRINT(PRT_ERROR, "travel type %d not implemented yet\n", reach->traveltype);
			return 8;
		} //end case
	} //end switch
...
#ifdef _DEBUG
	//BOTPRINT(PRT_MESSAGE, "%s: moving straight to goal\n", ClientName(ms->entitynum-1));
	//AAS_ClearShownDebugLines();
...
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags)
{
	...
	{
#ifdef _DEBUG
		BOTPRINT(PRT_MESSAGE, "client %d: movetogoal -> no goal\n", ms->client);
#endif //_DEBUG
		result->failure = qtrue;
		return;
	} //end if
	//BOTPRINT(PRT_MESSAGE, "numavoidreach = %d\n", ms->numavoidreach);
			...
			if (modelnum >= 0 && modelnum < MAX_MODELS)
			{
				...
				if (modeltype == MODELTYPE_FUNC_PLAT)
				{
						...
						(reach.facenum & 0x0000FFFF) != modelnum)
					{
						...
						if (reachnum)
						{
							//BOTPRINT(PRT_MESSAGE, "client %d: accidentally ended up on func_plat\n", ms->client);
							AAS_ReachabilityFromNum(reachnum, &reach);
						...
						else
						{
							if (bot_developer)
							{
								BOTPRINT(PRT_MESSAGE, "client %d: on func_plat without reachability\n", ms->client);
							} //end if
							result->blocked = qtrue;
				...
				else if (modeltype == MODELTYPE_FUNC_BOB)
				{
						...
						(reach.facenum & 0x0000FFFF) != modelnum)
					{
						...
						if (reachnum)
						{
							//BOTPRINT(PRT_MESSAGE, "client %d: accidentally ended up on func_bobbing\n", ms->client);
							AAS_ReachabilityFromNum(reachnum, &reach);
							ms->lastreachnum = reachnum;
							ms->reachability_time = AAS_Time() + BotReachabilityTime(&reach);
						} //end if
						else
						{
							if (bot_developer)
							{
								BOTPRINT(PRT_MESSAGE, "client %d: on func_bobbing without reachability\n", ms->client);
							} //end if
							result->blocked = qtrue;
							result->blockentity = ent;
							result->flags |= MOVERESULT_ONTOPOFOBSTACLE;
	...
	{
		//BOTPRINT(PRT_MESSAGE, "%s: onground, swimming or against ladder\n", ClientName(ms->entitynum-1));
		//
		...
		if (reachnum)
		{
			...
			else
			{
				...
				if (bot_developer)
				{
					if (ms->reachability_time < AAS_Time())
					{
						BOTPRINT(PRT_MESSAGE, "client %d: reachability timeout in ", ms->client);
					...
					{
						BOTPRINT(PRT_MESSAGE, "changed from area %d to %d\n", ms->lastareanum, ms->areanum);
					} //end if*/
				} //end if
#endif //_DEBUG
				...
				{
					reachnum = 0;
					//BOTPRINT(PRT_MESSAGE, "area change or timeout\n");
				} //end else if
			} //end else
		} //end if
		resultflags = 0;
		...
		if (!reachnum)
		{
			...
			if (!AAS_AreaReachability(ms->areanum))
			{
				...
				if (bot_developer)
				{
					BOTPRINT(PRT_MESSAGE, "area %d no reachability\n", ms->areanum);
				} //end if
#endif //_DEBUG
			} //end if
			...
			{
				BOTPRINT(PRT_MESSAGE, "goal not reachable\n");
			...
			if (bot_developer)
			{
				...
				if (ms->lastgoalareanum == goal->areanum)
				{
					...
					{
						BOTPRINT(PRT_MESSAGE, "same goal, going back to previous area\n");
					} //end if
				} //end if
			} //end if
#endif //_DEBUG
		...
		if (reachnum)
		{
			...
			//
#ifdef _DEBUG
			//BOTPRINT(PRT_MESSAGE, "client %d: ", ms->client);
			...
			switch(reach.traveltype & TRAVELTYPE_MASK)
			{
				...
				default:
				{
					BOTPRINT(PRT_FATAL, "travel type %d not implemented yet\n", (reach.traveltype & TRAVELTYPE_MASK));
					break;
				} //end case
			} //end switch
			result->traveltype = reach.traveltype;
		...
		if (bot_developer)
		{
			if (result->failure)
			{
				BOTPRINT(PRT_MESSAGE, "client %d: movement failure in ", ms->client);
				AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
				BOTPRINT(PRT_MESSAGE, "\n");
			} //end if
		} //end if
	...
	else
	{
		...
		for (i = numareas-1; i >= 0; i--)
		{
			...
			{
				//BOTPRINT(PRT_MESSAGE, "client %d used a jumppad without knowing, area %d\n", ms->client, areas[i]);
				foundjumppad = qtrue;
				...
				if (lastreachnum)
				{
					...
					//BOTPRINT(PRT_MESSAGE, "found jumppad reachability\n");
					break;
				} //end if
				else
				{
						...
						lastreachnum = AAS_NextAreaReachability(areas[i], lastreachnum))
					{
						...
						if ((reach.traveltype & TRAVELTYPE_MASK) == TRAVEL_JUMPPAD)
						{
							ms->lastreachnum = lastreachnum;
							ms->lastareanum = areas[i];
							//BOTPRINT(PRT_MESSAGE, "found jumppad reachability hard!!\n");
							break;
						} //end if
					} //end for
		...
		if (bot_developer)
		{
			...
			{
				BOTPRINT(PRT_MESSAGE, "client %d didn't find jumppad reachability\n", ms->client);
			} //end if
		} //end if
		//
		if (ms->lastreachnum)
		{
			//BOTPRINT(PRT_MESSAGE, "%s: NOT onground, swimming or against ladder\n", ClientName(ms->entitynum-1));
			AAS_ReachabilityFromNum(ms->lastreachnum, &reach);
			result->traveltype = reach.traveltype;
#ifdef _DEBUG
			//BOTPRINT(PRT_MESSAGE, "client %d finish: ", ms->client);
			//AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
			//BOTPRINT(PRT_MESSAGE, "\n");
#endif //_DEBUG
			//
			switch(reach.traveltype & TRAVELTYPE_MASK)
			{
				...
				default:
				{
					BOTPRINT(PRT_FATAL, "(last) travel type %d not implemented yet\n", (reach.traveltype & TRAVELTYPE_MASK));
					break;
				} //end case
			} //end switch
			result->traveltype = reach.traveltype;
			...
			if (bot_developer)
			{
				if (result->failure)
				{
					BOTPRINT(PRT_MESSAGE, "client %d: movement failure in finish ", ms->client);
					AAS_PrintTravelType(reach.traveltype & TRAVELTYPE_MASK);
					BOTPRINT(PRT_MESSAGE, "\n");
				} //end if
			} //end if
