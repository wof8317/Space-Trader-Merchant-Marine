...
typedef enum {
	...
	GT_CAMPAIGN,		// single player story
...
//level items
levelitem_t *levelitemheap;
levelitem_t *freelevelitems;
levelitem_t *levelitems;
int numlevelitems;
//map locations
maplocation_t *maplocations;
//camp spots
campspot_t *campspots;
//the game type
int g_gametype;
//additional dropped item weight
libvar_t *droppedweight;
...
bot_goalstate_t *BotGoalStateFromHandle(int handle)
{
	...
	{
		BOTPRINT(PRT_FATAL, "goal state handle %d out of range\n", handle);
		return NULL;
	} //end if
	if (!botgoalstates[handle])
	{
		BOTPRINT(PRT_FATAL, "invalid goal state %d\n", handle);
		return NULL;
	} //end if
...
itemconfig_t *LoadItemConfig(char *filename)
{
	...
	if (max_iteminfo < 0)
	{
		BOTPRINT(PRT_ERROR, "max_iteminfo = %d\n", max_iteminfo);
		max_iteminfo = 256;
	...
	FreeSource(source);
	//
	if (!ic->numiteminfo) BOTPRINT(PRT_WARNING, "no item info loaded\n");
...
levelitem_t *AllocLevelItem(void)
{
	...
	if (!li)
	{
		BOTPRINT(PRT_FATAL, "out of level items\n");
		return NULL;
	} //end if
	//
...
void BotInitInfoEntities(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		else if (!strcmp(classname, "info_camp"))
		{
			...
			if (!cs->areanum)
			{
				BOTPRINT(PRT_MESSAGE, "camp spot at %1.1f %1.1f %1.1f in solid\n", cs->origin[0], cs->origin[1], cs->origin[2]);
				FreeMemory(cs);
				continue;
			} //end if
			cs->next = campspots;
	...
	if (bot_developer)
	{
		BOTPRINT(PRT_MESSAGE, "%d map locations\n", numlocations);
		BOTPRINT(PRT_MESSAGE, "%d camp spots\n", numcampspots);
	} //end if
...
void BotInitLevelItems(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		{
			BOTPRINT(PRT_ERROR, "item %s without origin\n", classname);
			continue;
		} //end else
		//
		goalareanum = 0;
		...
		if (spawnflags & 1)
		{
			...
			if (!(AAS_PointContents(origin) & CONTENTS_WATER))
			{
				...
				if (trace.fraction >= 1)
				{
					...
					//BOTPRINT(PRT_MESSAGE, "item %s reachable from jumppad area %d\r\n", ic->iteminfo[i].classname, goalareanum);
		...
		else
		{
			...
			if (!li->goalareanum)
			{
				BOTPRINT(PRT_MESSAGE, "%s not reachable for bots at (%1.1f %1.1f %1.1f)\n",
												classname, origin[0], origin[1], origin[2]);
			} //end if
		} //end else
		//
	...
	BOTPRINT(PRT_MESSAGE, "found %d level items\n", numlevelitems);
...
void BotUpdateEntityItems(void)
{
	...
	for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent))
	{
		...
		AddLevelItemToList(li);
		//BOTPRINT(PRT_MESSAGE, "found new level item %s\n", ic->iteminfo[i].classname);
	} //end for
	/*
	for (li = levelitems; li; li = li->next)
	{
...
void BotPushGoal(int goalstate, bot_goal_t *goal)
{
	...
	{
		BOTPRINT(PRT_ERROR, "goal heap overflow\n");
	...
	if (!bestitem)
	{
		...
		if (!AAS_AreaLava(areanum) && !AAS_AreaSlime(areanum))
		{
			if (AAS_RandomGoalArea(areanum, travelflags, &goal.areanum, goal.origin))
			{
				...
				//
#ifdef _DEBUG
				BOTPRINT(PRT_MESSAGE, "chosen roam goal area %d\n", goal.areanum);
#endif //_DEBUG
				return qtrue;
			} //end if
		} //end if
...
int BotLoadItemWeights(int goalstate, char *filename)
{
	...
	if (!gs->itemweightconfig)
	{
		BOTPRINT(PRT_FATAL, "couldn't load weights\n");
...
void BotFreeGoalState(int handle)
{
	...
	if (!botgoalstates[handle])
	{
		BOTPRINT(PRT_FATAL, "invalid goal state handle %d\n", handle);
		return;
	} //end if
...
int BotSetupGoalAI(void)
{
	...
	if (!itemconfig)
	{
		BOTPRINT(PRT_FATAL, "couldn't load item config\n");
