...
void AAS_ShowFace(int facenum)
{
	...
	{
		BOTPRINT(PRT_ERROR, "facenum %d out of range\n", facenum);
	} //end if
	...
	for (i = 0; i < face->numedges; i++)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "edgenum %d out of range\n", edgenum);
		} //end if
...
void AAS_ShowArea(int areanum, int groundfacesonly)
{
	...
	{
		BOTPRINT(PRT_ERROR, "area %d out of range [0, %d]\n",
