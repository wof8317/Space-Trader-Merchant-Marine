...
memoryblock_t *BlockFromPointer(void *ptr, char *str)
{
	...
	if (!ptr)
	{
		...
		//crash[0] = 1;
		BOTPRINT(PRT_FATAL, "%s: NULL pointer\n", str);
#endif // MEMDEBUG
		return NULL;
	} //end if
	...
	{
		BOTPRINT(PRT_FATAL, "%s: invalid memory block\n", str);
		return NULL;
	} //end if
	if (block->ptr != ptr)
	{
		BOTPRINT(PRT_FATAL, "%s: memory block pointer invalid\n", str);
		return NULL;
	} //end if
	return block;
...
{
	BOTPRINT(PRT_MESSAGE, "total allocated memory: %d KB\n", allocatedmemory >> 10);
	BOTPRINT(PRT_MESSAGE, "total botlib memory: %d KB\n", totalmemorysize >> 10);
	BOTPRINT(PRT_MESSAGE, "total memory blocks: %d\n", numblocks);
