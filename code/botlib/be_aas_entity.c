...
int AAS_UpdateEntity(int entnum, bot_entitystate_t *state)
{
	...
	if (!aasworld.loaded)
	{
		BOTPRINT(PRT_MESSAGE, "AAS_UpdateEntity: not loaded\n");
...
void AAS_EntityInfo(int entnum, aas_entityinfo_t *info)
{
	...
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityInfo: aasworld not initialized\n");
	...
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityInfo: entnum %d out of range\n", entnum);
		Com_Memset(info, 0, sizeof(aas_entityinfo_t));
		return;
	} //end if
...
void AAS_EntityOrigin(int entnum, vec3_t origin)
{
	if (entnum < 0 || entnum >= aasworld.maxentities)
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityOrigin: entnum %d out of range\n", entnum);
		VectorClear(origin);
		return;
	} //end if
...
int AAS_EntityModelindex(int entnum)
{
	if (entnum < 0 || entnum >= aasworld.maxentities)
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityModelindex: entnum %d out of range\n", entnum);
		return 0;
	} //end if
...
int AAS_EntityType(int entnum)
{
	...
	if (entnum < 0 || entnum >= aasworld.maxentities)
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityType: entnum %d out of range\n", entnum);
		return 0;
	} //end if
...
int AAS_EntityModelNum(int entnum)
{
	...
	if (entnum < 0 || entnum >= aasworld.maxentities)
	{
		BOTPRINT(PRT_FATAL, "AAS_EntityModelNum: entnum %d out of range\n", entnum);
		return 0;
	} //end if
	return aasworld.entities[entnum].i.modelindex;
...
void AAS_EntitySize(int entnum, vec3_t mins, vec3_t maxs)
{
	...
	if (entnum < 0 || entnum >= aasworld.maxentities)
	{
		BOTPRINT(PRT_FATAL, "AAS_EntitySize: entnum %d out of range\n", entnum);
		return;
	} //end if
