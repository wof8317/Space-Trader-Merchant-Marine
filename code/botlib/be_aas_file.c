...
{
	if( aasworld.is_fast_loaded )
	{
		Com_Memset( &aasworld, 0, sizeof( aasworld ) );
		return;
	}
...
void AAS_FileInfo(void)
{
	...
	BOTPRINT(PRT_MESSAGE, "version = %d\n", AASVERSION);
	BOTPRINT(PRT_MESSAGE, "numvertexes = %d\n", aasworld.numvertexes);
	BOTPRINT(PRT_MESSAGE, "numplanes = %d\n", aasworld.numplanes);
	BOTPRINT(PRT_MESSAGE, "numedges = %d\n", aasworld.numedges);
	BOTPRINT(PRT_MESSAGE, "edgeindexsize = %d\n", aasworld.edgeindexsize);
	BOTPRINT(PRT_MESSAGE, "numfaces = %d\n", aasworld.numfaces);
	BOTPRINT(PRT_MESSAGE, "faceindexsize = %d\n", aasworld.faceindexsize);
	BOTPRINT(PRT_MESSAGE, "numareas = %d\n", aasworld.numareas);
	BOTPRINT(PRT_MESSAGE, "numareasettings = %d\n", aasworld.numareasettings);
	BOTPRINT(PRT_MESSAGE, "reachabilitysize = %d\n", aasworld.reachabilitysize);
	BOTPRINT(PRT_MESSAGE, "numnodes = %d\n", aasworld.numnodes);
	BOTPRINT(PRT_MESSAGE, "numportals = %d\n", aasworld.numportals);
	BOTPRINT(PRT_MESSAGE, "portalindexsize = %d\n", aasworld.portalindexsize);
	BOTPRINT(PRT_MESSAGE, "numclusters = %d\n", aasworld.numclusters);
	//
	for (n = 0, i = 0; i < aasworld.numareasettings; i++)
	{
		if (aasworld.areasettings[i].areaflags & AREA_GROUNDED) n++;
	} //end for
	BOTPRINT(PRT_MESSAGE, "num grounded areas = %d\n", n);
	//
	BOTPRINT(PRT_MESSAGE, "planes size %d bytes\n", aasworld.numplanes * sizeof(aas_plane_t));
	BOTPRINT(PRT_MESSAGE, "areas size %d bytes\n", aasworld.numareas * sizeof(aas_area_t));
	BOTPRINT(PRT_MESSAGE, "areasettings size %d bytes\n", aasworld.numareasettings * sizeof(aas_areasettings_t));
	BOTPRINT(PRT_MESSAGE, "nodes size %d bytes\n", aasworld.numnodes * sizeof(aas_node_t));
	BOTPRINT(PRT_MESSAGE, "reachability size %d bytes\n", aasworld.reachabilitysize * sizeof(aas_reachability_t));
	BOTPRINT(PRT_MESSAGE, "portals size %d bytes\n", aasworld.numportals * sizeof(aas_portal_t));
	BOTPRINT(PRT_MESSAGE, "clusters size %d bytes\n", aasworld.numclusters * sizeof(aas_cluster_t));
	...
	optimized = aasworld.numplanes * sizeof(aas_plane_t) +
					...
					aasworld.numportals * sizeof(aas_portal_t) +
					aasworld.numclusters * sizeof(aas_cluster_t);
	BOTPRINT(PRT_MESSAGE, "optimzed size %d KB\n", optimized >> 10);
...
char *AAS_LoadAASLump(fileHandle_t fp, int offset, int length, int *lastoffset, int size)
{
	...
	{
		BOTPRINT(PRT_WARNING, "AAS file not sequentially read\n");
...
int AAS_LoadAASFileFast( void *pAasData, const char *filename )
{
	char *buf;
	int offset, length;
	aas_header_t header;

	BOTPRINT( PRT_MESSAGE, "trying to load %s\n", filename );
	...
	if( aasworld.bspchecksum != -1 && LittleLong( header.bspchecksum ) != aasworld.bspchecksum )
	{
	...
	aasworld.bboxes = (aas_bbox_t*)(buf + offset);
	...
	aasworld.vertexes = (aas_vertex_t*)(buf + offset);
	...
	aasworld.planes = (aas_plane_t*)(buf + offset);
	...
	aasworld.edges = (aas_edge_t*)(buf + offset);
	...
	aasworld.edgeindex = (aas_edgeindex_t*)(buf + offset);
	...
	aasworld.faces = (aas_face_t*)(buf + offset);
	...
	aasworld.faceindex = (aas_faceindex_t*)(buf + offset);
	...
	aasworld.areas = (aas_area_t*)(buf + offset);
	...
	aasworld.areasettings = (aas_areasettings_t*)(buf + offset);
	...
	aasworld.reachability = (aas_reachability_t*)(buf + offset);
	...
	aasworld.nodes = (aas_node_t*)(buf + offset);
	...
	aasworld.portals = (aas_portal_t*)(buf + offset);
	...
	aasworld.portalindex = (aas_portalindex_t*)(buf + offset);
	...
	aasworld.clusters = (aas_cluster_t*)(buf + offset);
...
qboolean AAS_WriteAASFile(char *filename)
{
	...
	BOTPRINT(PRT_MESSAGE, "writing %s\n", filename);
	//swap the aas data
	AAS_SwapAASData();
	...
	if (!fp)
	{
		BOTPRINT(PRT_ERROR, "error opening %s\n", filename);
		return qfalse;
	} //end if
	//write the header
