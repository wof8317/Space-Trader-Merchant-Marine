...
char *AAS_StringFromIndex(char *indexname, char *stringindex[], int numindexes, int index)
{
	...
	{
		BOTPRINT(PRT_ERROR, "%s: index %d not setup\n", indexname, index);
		return "";
	} //end if
	if (index < 0 || index >= numindexes)
	{
		BOTPRINT(PRT_ERROR, "%s: index %d out of range\n", indexname, index);
		return "";
	} //end if
	if (!stringindex[index])
	{
		if (index)
		{
			BOTPRINT(PRT_ERROR, "%s: reference to unused index %d\n", indexname, index);
		} //end if
		return "";
	} //end if
...
int AAS_IndexFromString(char *indexname, char *stringindex[], int numindexes, char *string)
{
	...
	if (!aasworld.indexessetup)
	{
		BOTPRINT(PRT_ERROR, "%s: index not setup \"%s\"\n", indexname, string);
		return 0;
	} //end if
...
void AAS_SetInitialized(void)
{
	...
	BOTPRINT(PRT_MESSAGE, "AAS initialized.\n");
#ifdef _DEBUG
...
void AAS_ContinueInit(float time)
{
	...
	if (aasworld.savefile || ((int)LibVarGetValue("forcewrite")))
	{
		...
		{
			BOTPRINT(PRT_MESSAGE, "%s written succesfully\n", aasworld.filename);
		} //end if
		else
		{
			BOTPRINT(PRT_ERROR, "couldn't write %s\n", aasworld.filename);
		} //end else
	} //end if
...
// Changes Globals:		-
//===========================================================================
int AAS_LoadFiles( void *pAasData, const char *mapname )
{
	int errnum;
...
//	char bspfile[MAX_PATH];
	...
	if( pAasData )
		errnum = AAS_LoadAASFileFast( pAasData, aasfile );
	else
...
// Changes Globals:		-
//===========================================================================
int AAS_LoadMap( void *pAasData, const char *mapname )
{
	int	errnum;
	...
	//load the map
	errnum = AAS_LoadFiles( pAasData, mapname );
	if (errnum != BLERR_NOERROR)
	{
...
void AAS_Shutdown(void)
{
	...
	//print shutdown
	BOTPRINT(PRT_MESSAGE, "AAS shutdown.\n");
...
void Q_EXTERNAL_CALL AAS_DrawDebugSurface( void (Q_EXTERNAL_CALL *drawPoly)(int color, int numPoints, float *points) ) {
	...
	for (i = 0; i < aasworld.numareas; i++) {
		vec3_t v[4];
		v[0][0] = aasworld.areas[i].mins[0];
		v[0][1] = aasworld.areas[i].mins[1];
		v[0][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;

		v[1][0] = aasworld.areas[i].mins[0];
		v[1][1] = aasworld.areas[i].mins[1];
		v[1][2] = aasworld.areas[i].maxs[2] - AAS_FUDGE;

		v[2][0] = aasworld.areas[i].mins[0];
		v[2][1] = aasworld.areas[i].maxs[1];
		v[2][2] = aasworld.areas[i].maxs[2] - AAS_FUDGE;

		v[3][0] = aasworld.areas[i].mins[0];
		v[3][1] = aasworld.areas[i].maxs[1];
		v[3][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;
		...
		v[1][0] = aasworld.areas[i].maxs[0];
		v[1][1] = aasworld.areas[i].mins[1];
		v[1][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;

		v[2][0] = aasworld.areas[i].maxs[0];
		v[2][1] = aasworld.areas[i].maxs[1];
		v[2][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;
		...
		v[2][0] = aasworld.areas[i].maxs[0];
		v[2][1] = aasworld.areas[i].mins[1];
		v[2][2] = aasworld.areas[i].maxs[2] - AAS_FUDGE;

		v[3][0] = aasworld.areas[i].mins[0];
		v[3][1] = aasworld.areas[i].mins[1];
		v[3][2] = aasworld.areas[i].maxs[2] - AAS_FUDGE;

		drawPoly(0, 4, v[0]);

		v[0][0] = aasworld.areas[i].maxs[0];
		v[0][1] = aasworld.areas[i].mins[1];
		v[0][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;

		v[1][0] = aasworld.areas[i].maxs[0];
		v[1][1] = aasworld.areas[i].maxs[1];
		v[1][2] = aasworld.areas[i].mins[2] - AAS_FUDGE;
		...
		v[0][0] = aasworld.areas[i].mins[0];
		v[0][1] = aasworld.areas[i].mins[1];
		v[0][2] = aasworld.areas[i].maxs[2] - AAS_FUDGE;
