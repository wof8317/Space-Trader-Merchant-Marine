...
void AAS_PresenceTypeBoundingBox(int presencetype, vec3_t mins, vec3_t maxs)
{
	...
	{
		BOTPRINT(PRT_FATAL, "AAS_PresenceTypeBoundingBox: unknown presence type\n");
		index = 2;
	} //end if
...
#endif
		{
			BOTPRINT(PRT_FATAL, "empty aas link heap\n");
		} //end if
		return NULL;
	} //end if
...
int AAS_PointAreaNum(vec3_t point)
{
	...
	if (!aasworld.loaded)
	{
		BOTPRINT(PRT_ERROR, "AAS_PointAreaNum: aas not loaded\n");
		return 0;
	} //end if
	...
	while (nodenum > 0)
	{
//		BOTPRINT(PRT_MESSAGE, "[%d]", nodenum);
#ifdef AAS_SAMPLE_DEBUG
		if (nodenum >= aasworld.numnodes)
		{
			BOTPRINT(PRT_ERROR, "nodenum = %d >= aasworld.numnodes = %d\n", nodenum, aasworld.numnodes);
			return 0;
		} //end if
		...
		{
			BOTPRINT(PRT_ERROR, "node->planenum = %d >= aasworld.numplanes = %d\n", node->planenum, aasworld.numplanes);
			return 0;
		} //end if
#endif //AAS_SAMPLE_DEBUG
...
int AAS_AreaCluster(int areanum)
{
	...
	{
		BOTPRINT(PRT_ERROR, "AAS_AreaCluster: invalid area number\n");
		return 0;
	} //end if
...
int AAS_AreaPresenceType(int areanum)
{
	...
	if (areanum <= 0 || areanum >= aasworld.numareas)
	{
		BOTPRINT(PRT_ERROR, "AAS_AreaPresenceType: invalid area number\n");
		return 0;
	} //end if
																				...
																				int passent)
{
	...
	while (1)
	{
		...
		if (nodenum < 0)
		{
			...
			{
				BOTPRINT(PRT_ERROR, "AAS_TraceBoundingBox: -nodenum out of range\n");
				return trace;
			} //end if
#endif //AAS_SAMPLE_DEBUG
			//BOTPRINT(PRT_MESSAGE, "areanum = %d, must be %d\n", -nodenum, AAS_PointAreaNum(start));
...
//			VectorSubtract(end, start, v1);
		...
		if ((front >= -ON_EPSILON && back >= -ON_EPSILON))
		{
			...
			{
				BOTPRINT(PRT_ERROR, "AAS_TraceBoundingBox: stack overflow\n");
				return trace;
			} //end if
		} //end if
...
int AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points, int maxareas)
{
	...
	while (1)
	{
		...
		if (nodenum < 0)
		{
			...
			if (-nodenum > aasworld.numareasettings)
			{
				BOTPRINT(PRT_ERROR, "AAS_TraceAreas: -nodenum = %d out of range\n", -nodenum);
				return numareas;
			} //end if
#endif //AAS_SAMPLE_DEBUG
			//BOTPRINT(PRT_MESSAGE, "areanum = %d, must be %d\n", -nodenum, AAS_PointAreaNum(start));
		...
		if (nodenum > aasworld.numnodes)
		{
			BOTPRINT(PRT_ERROR, "AAS_TraceAreas: nodenum out of range\n");
			return numareas;
		} //end if
#endif //AAS_SAMPLE_DEBUG
		//the node to test against
		...
		if (front > 0 && back > 0)
		{
			...
			if (tstack_p >= &tracestack[127])
			{
				BOTPRINT(PRT_ERROR, "AAS_TraceAreas: stack overflow\n");
				return numareas;
			} //end if
		} //end if
		//if the whole to be traced line is totally at the back of this node
...
qboolean AAS_InsideFace(aas_face_t *face, vec3_t pnormal, vec3_t point, float epsilon)
{
	...
	for (i = 0; i < face->numedges; i++)
	{
		...
		{
			BOTPRINT(PRT_MESSAGE, "winding not counter clockwise\n");
		} //end if
...
aas_link_t *AAS_AASLinkEntity(vec3_t absmins, vec3_t absmaxs, int entnum)
{
	...
	if (!aasworld.loaded)
	{
		BOTPRINT(PRT_ERROR, "AAS_LinkEntity: aas not loaded\n");
		return NULL;
	} //end if
	...
	while (1)
	{
		...
		{
			BOTPRINT(PRT_ERROR, "AAS_LinkEntity: stack overflow\n");
			break;
		} //end if
...
int AAS_AreaInfo( int areanum, aas_areainfo_t *info )
{
	...
	if (areanum <= 0 || areanum >= aasworld.numareas)
	{
		BOTPRINT(PRT_ERROR, "AAS_AreaInfo: areanum %d out of range\n", areanum);
		return 0;
	} //end if
