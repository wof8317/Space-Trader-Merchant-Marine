...
void BotReportStatus(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		...
		case LTG_FOLLOWPATH:
		{
		    BotAI_Print(PRT_MESSAGE, "%-20s%s%s: follow path\n", netname, leader, flagstatus);
		    break;
		}
		case LTG_GETFLAG:
		{
...
void BotSetInfoConfigString(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		...
		case LTG_FOLLOWPATH:
		{
		    Com_sprintf(action, sizeof(action), "following path");
		    break;
		}
		case LTG_GETFLAG:
		{
...
char *BotGetSessionString() {
    static char buf[MAX_INFO_STRING];
    int i;
    bot_state_t *bs;
	
    for (i=0; i < maxclients; i++) {
        bs = botstates[i];
        //Skip if bs->bottype is BOT_NORMAL
        if ( !bs || !bs->inuse || bs->bottype == BOT_NORMAL)
            continue;
        Info_SetValueForKey(buf, va("%s_spent",level.clients[bs->client].pers.netname),va("%d", bs->spent));
        //If the bot is marked as saved, store that in the variable
        if (bs->saved == qtrue) {
            Info_SetValueForKey(buf, va("%s_saved",level.clients[bs->client].pers.netname),va("%d", qtrue));
        }
    }
    return buf;
}
...
void BotSetSpent(int bot, qboolean spent) {
	if (botstates[bot])
    	botstates[bot]->spent = spent;
}

qboolean BotGetSpent(int bot) {
    return botstates[bot]->spent;
}
...
int BotGetType( int entnum )
{
	...
	if (entnum >= MAX_CLIENTS) return -1;
...
void BotDoneTalking(int bot, int delay, gentity_t *dest)
{
	if (botstates[bot] )
	{
    	//Resets the stand_time variable for this bot
    	botstates[bot]->stand_time = FloatTime() + delay;
		//BotAI_BotInitialChat(botstates[bot], "goodbye", dest->client->pers.netname, NULL);
		trap_BotEnterChat(botstates[bot]->cs, dest->s.number, CHAT_TELL);
    }
}
...
void BotSetIdealViewAngle(int bot, vec3_t angle)
{
	bot_state_t *bs;
	bs = botstates[bot];
	VectorCopy(angle, bs->ideal_viewangles);
	VectorCopy(angle, bs->spawn_viewangles);
}
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
	...
	for (i = 0; i < 2; i++) {
		...
		if (bot_challenge.integer) {
			...
			bs->viewangles[i] = AngleMod(bs->viewangles[i]);
			//demping
			bs->viewanglespeed[i] *= 0.35f * (1.0f - factor);
		}
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time) {
	...
	if (bi->actionflags & ACTION_GRENADE) ucmd->buttons |= BUTTON_GRENADE;
	...
	ucmd->forwardmove = DotProduct(forward, bi->dir) * bi->speed * 0.85f;
	ucmd->rightmove = DotProduct(right, bi->dir) * bi->speed * 0.5f;
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	char filename[MAX_PATH], name[MAX_PATH], gender[MAX_PATH], userinfo[MAX_INFO_STRING], botinfo[MAX_INFO_STRING];
	char *botname, *bottype;
	gentity_t *bot_spawn;
	bot_state_t *bs;
	int errnum;
	...
	bs->weaponnum = WP_HIGHCAL;
	if ( g_gametype.integer != GT_HUB ) {
		bs->walker = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_WALKER, 0, 1);
	} else { //bots walk while in the hub
		bs->walker = 1.0f;
	}
	numbots++;

	trap_GetUserinfo(bs->client, userinfo, MAX_INFO_STRING);
	
	
	bottype = Info_ValueForKey(userinfo, "bottype");
	botname = Info_ValueForKey(userinfo, "name");
	
	trap_Cvar_VariableStringBuffer(va("%s_bots",level.mapname), botinfo, MAX_INFO_STRING);
	
	bs->bottype = BG_GetBotType( bottype );
		
	//Set our special flags if it's not a normal bot
	if (bs->bottype != BOT_NORMAL)
	{
	    //Get the bot going on it's path
	    bot_spawn = G_Find(NULL,FOFS(targetname), botname);
	    if (bs->bottype == BOT_INFORMANT && bot_spawn)
	    {
			bs->curpatrolpoint = BotFindGlobalWayPoint(bot_spawn->target);
            bs->ltgtype = LTG_FOLLOWPATH;
	    }
	    if (bot_spawn){
	        bs->specialflags = bot_spawn->spawnflags;
	    }
	    if (bs->bottype == BOT_BOSS) {
            //Implicit END_LEVEL_ON_KILL for bosses
	        bs->specialflags |= BSF_END_LEVEL_ON_KILL;
	        SetLeader( TEAM_RED, client );
	    }

	    bs->spent = atoi(Info_ValueForKey(botinfo, va("%s_spent", botname)));
	}
...
void BotResetState(bot_state_t *bs) {
	...
	int movestate, goalstate, chatstate, weaponstate, bottype, spent, saved;
	...
	//Save the bot type, if they were spent, and if they were saved
	bottype = bs->bottype;
	spent = bs->spent;
	saved = bs->saved;
...
int BotAIShutdown( int restart ) {
	...
	if ( restart ) {
	    ...
	    if (g_botsReconnect.integer == 0 )
	    {
		    for (i = 0; i < MAX_CLIENTS; i++) {
		        if (botstates[i]) {
    				trap_SetUserinfo(botstates[i]->client, "");
	    		}
		    }
		}
		trap_BotLibShutdown();
