...
static void PlayerIntroSound( const char *modelAndSkin ) {
	...
	trap_SendConsoleCommand( EXEC_APPEND, va( "play sound/player/announce/%s.ogg\n", skin ) );
}
...
void G_AddBot( const char *type, const char *model, const char *name, int npcId, float skill, const char *team, int delay, gentity_t *spawn) {
	...
	if ( !botinfo )
		botinfo = G_GetBotInfoByName( type );
	...
	if (skill > 5.0) skill = 5.0;

	Info_SetValueForKey( userinfo, "bottype", type );
	...
	if ( skill >= 1 && skill < 2 ) {
		Info_SetValueForKey( userinfo, "handicap", "10" );
	}
	...
	clientNum = trap_BotAllocateClient( BG_GetBotType( type ) == BOT_PLAYER );
	if ( clientNum == -1 ) {
	...
	Info_SetValueForKey( userinfo, "id", va( "%d", npcId ) );
...
void Svcmd_AddBot_f( void ) {
	...
	G_AddBot( "", NULL, name, -1, skill, team, delay, NULL );
...
void G_InitBots( qboolean restart ) {
	...
	if ( g_usemapspawns.integer )
	{
		trap_SendConsoleCommand( EXEC_INSERT, "wait ; st_bots ;" );
	}
}
