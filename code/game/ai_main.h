...
#define LTG_FOLLOWPATH  			16  //Follow a path
...
typedef struct bot_state_s
{
    ...
    float wait_time;                                //time the bot is waiting still   
	...
	float idle_time;								//time the bot has been idle for
	...
	int bottype;                                    //type of the bot
	int specialflags;                               //special flags to control special events that
	                                                //happen with the bot.  Ex: level ends when they are killed
	qboolean spent;
	qboolean saved;
} bot_state_t;
