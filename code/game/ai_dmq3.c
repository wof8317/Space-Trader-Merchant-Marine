...
#define IDEAL_ATTACKDIST			140.0f
...
void BotUpdateInventory(bot_state_t *bs) {
	...
	//armor
	bs->inventory[INVENTORY_ARMOR] = bs->cur_ps.shields;
	//weapons
	...
	bs->inventory[INVENTORY_ASSAULT] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_ASSAULT)) != 0;
	bs->inventory[INVENTORY_GRAPPLINGHOOK] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GRAPPLING_HOOK)) != 0;
    bs->inventory[INVENTORY_HIGHCAL] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_HIGHCAL)) != 0;
	bs->inventory[INVENTORY_C4] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_C4)) != 0;;
	
	//ammo
	bs->inventory[INVENTORY_SHELLS] = bs->cur_ps.ammo[WP_SHOTGUN];
	bs->inventory[INVENTORY_BULLETS] = bs->cur_ps.ammo[WP_PISTOL];
	bs->inventory[INVENTORY_GRENADES] = bs->cur_ps.ammo[WP_GRENADE];
	bs->inventory[INVENTORY_CLIPS] = bs->cur_ps.ammo[WP_ASSAULT];
    bs->inventory[INVENTORY_HIGHCALAMMO] = bs->cur_ps.ammo[WP_HIGHCAL];
	bs->inventory[INVENTORY_C4] = bs->cur_ps.ammo[WP_C4];
...
==================
*/
int BotCanSeePlayer(bot_state_t *bs) {
    int i;
    vec3_t delta, dir;
    float dist, vis, spam_dist, spam_chance, spam_chance_to_beat;
    char netname[MAX_MESSAGE_SIZE];
    if (bot_nochat.integer) return qfalse;
    if (BotIsObserver(bs)) return qfalse;
    //No bot spamage unless we are on the hub map.
    if (g_gametype.integer != GT_HUB) return qfalse;
    
    //Traders can't see players, they just trade.
	if (bs->bottype == BOT_TRADER) return qfalse;
    for(i=0; i < level.maxclients; i++)
    {
		if (g_entities[i].inuse == qfalse || //not in use
			(g_entities[i].r.svFlags & SVF_BOT) == SVF_BOT || //a bot
			i == bs->client //is ourself, self
			) //not already talking
		{//skip this check
			continue;
		}
		spam_chance = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_SPAM_CHANCE, 0, 4000);
		spam_chance_to_beat = random();
        if (spam_chance < spam_chance_to_beat ) { 
			VectorSubtract( bs->eye, level.clients[i].ps.origin, delta );
			dist = VectorLength( delta );
			spam_dist = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_SPAM_DIST, 0, 4000);
			if (dist < spam_dist) {
		
				//Check to see if the player is inside of a 270 degree view.  This lets you walk behind an informant, but not much more
				vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 270, i);
				
				if ( vis > 0.999999f )
				{
					strncpy(netname, level.clients[i].pers.netname, MAX_MESSAGE_SIZE);
					//Get the vector between the end of the trace and the bots origin
					VectorSubtract(level.clients[i].ps.origin, bs->origin, dir);
					//Convert that vector to the angles to look in that direction
					vectoangles(dir, bs->ideal_viewangles);
					bs->ideal_viewangles[PITCH]=0;
				    //If it's been less then 5 seconds, return
				    /* As per bug 6433, disabling this
    				if (bs->lastchat_time < FloatTime()) {
						BotAI_BotInitialChat(bs, "locational_spam", netname, NULL);
						trap_BotEnterChat(bs->cs, i, CHAT_TELL);
						//G_Voice( &g_entities[bs->entitynum], &g_entities[i], SAY_TELL, "greeting", qtrue);
	
						bs->lastchat_time = FloatTime()+20.0f;
					}*/
					bs->wait_time=0;
					return qtrue;
				}
...
// is a player within distance
qboolean BotPlayerInView(bot_state_t *bs, int distance) {
	int i;
	distance = Square(distance);
	for (i=0; i < MAX_PLAYERS; i++)
	{
		// check for a player in a 360 degree angle
		float vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, i);
		
		if ( vis >= 0.999f )
		{
			vec3_t dir;
			VectorSubtract(level.clients[i].ps.origin, bs->eye, dir);
			if (VectorLength(dir) < distance)
				return qtrue;
		}
	}
	return qfalse;
...
qboolean BotIntermission(bot_state_t *bs) {
	...
	if (level.intermissiontime && gs.INPLAY) return qtrue;
	return ( bs->cur_ps.pm_type == PM_INTERMISSION);
}
...
/*
==================
BotFindGlobalWayPoint
==================
*/
bot_waypoint_t *BotFindGlobalWayPoint(char *name) {
	int i;
	
	for(i=0;i< MAX_WAYPOINTS; i++)
	{
		if (!Q_stricmp(botai_waypoints[i].name,name) ) return &botai_waypoints[i];
	}
	return NULL;
}
...
==================
*/
void BotLoadWaypoints(void) {
	gentity_t *path;
	path = NULL;
	while ( (path = G_Find( path, FOFS( classname ), "info_botpath" )) != NULL){
		BotCreateWayPoint(path->targetname, path->s.origin, BotPointAreaNum(path->s.origin));
	}
}
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_ASSAULT] > 0 &&
			bs->inventory[INVENTORY_CLIPS] > 0) return 85;
	//if the bot can use the grenade launcher
	if (bs->inventory[INVENTORY_GRENADE] > 0 &&
			bs->inventory[INVENTORY_GRENADES] > 10) return 80;
    //if the bot can use the highcal
    if (bs->inventory[INVENTORY_HIGHCAL] > 0 &&
            bs->inventory[INVENTORY_HIGHCALAMMO] > 5) return 60;
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs) {
	...
	//
	if (bs->inventory[INVENTORY_C4] > 0 &&
			bs->inventory[INVENTORY_MINES] > 5) return qtrue;
	//if the bot can use the assault rifle
	if (bs->inventory[INVENTORY_ASSAULT] > 0 &&
			bs->inventory[INVENTORY_CLIPS] > 0) return qtrue;
    //if the bot can use the high cal
    if (bs->inventory[INVENTORY_HIGHCAL] > 0 &&
            bs->inventory[INVENTORY_HIGHCALAMMO] > 5) return qtrue;
	return qfalse;
}
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if (movetype == MOVE_JUMP) {
		...
		if (bs->attackjump_time > FloatTime()) {
			...
			bs->attackjump_time = FloatTime() + 1.0f;
		}
	}
	...
	if (attack_skill <= 0.4f) {
		...
		if (dist > attack_dist + attack_range) {
			if (trap_BotMoveInDirection(bs->ms, forward, 400.0f, movetype)) return moveresult;
		}
		if (dist < attack_dist - attack_range) {
			if (trap_BotMoveInDirection(bs->ms, backward, 400.0f, movetype)) return moveresult;
		}
		return moveresult;
	}
	...
	strafechange_time = 0.4f + (1.0f - attack_skill) * 0.2f;
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
	...
	if (!InFieldOfVision(viewangles, fov, entangles)) return 0.0f;
	//
	...
	for (i = 0; i < 3; i++) {
		...
		if (trace.fraction >= 1.0f || trace.ent == hitent) {
			...
			if (bestvis >= 0.95f) return bestvis;
		}
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		//if we are in GT_HOSTAGE, don't consider other bots as possible enemies(so bots on the other team don't shoot the hostage)
		if ( g_gametype.integer == GT_HOSTAGE && g_entities[entinfo.number].r.svFlags & SVF_BOT) continue;
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	//
	aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL, 0.0f, 1.0f);
	aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY, 0.0f, 1.0f);
	//
	if (aim_skill > 0.95f) {
		//don't aim too early
		reactiontime = 0.5f * trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0.0f, 1.0f);
	...
	if (wi.number == WP_PISTOL) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_PISTOL, 0.0f, 1.0f);
	}
	else if (wi.number == WP_SHOTGUN) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0.0f, 1.0f);
	}
	else if (wi.number == WP_GRENADE) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADE, 0.0f, 1.0f);
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_GRENADE, 0.0f, 1.0f);
	}
	else if (wi.number == WP_ASSAULT) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_ASSAULT, 0.0f, 1.0f);
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_ASSAULT, 0.0f, 1.0f);
	}
    else if (wi.number == WP_HIGHCAL) {
        aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_HIGHCAL, 0.0f, 1.0f);
    }
	//
	if (aim_accuracy <= 0.0f) aim_accuracy = 0.0001f;
	//get the enemy entity information
	BotEntityInfo(bs->enemy, &entinfo);
	...
	VectorScale(enemyvelocity, 1.0f / entinfo.update_time, enemyvelocity);
	...
	if (aim_skill < 0.9f) {
		...
		if (VectorLengthSquared(dir) > Square(48.0f)) {
			//if the enemy changed direction
			if (DotProduct(bs->enemyvelocity, enemyvelocity) < 0.0f) {
	...
	enemyvisible = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360.0f, bs->enemy);
	...
	if (enemyvisible) {
		...
		if (wi.speed) {
			...
			if (!(dist > 100.0f && VectorLengthSquared(dir) < Square(32))) {
				...
				if (aim_skill > 0.8f &&
					...
					trap_AAS_PredictClientMovement(&move, bs->enemy, origin,
														...
														dir, cmdmove, 0,
														dist * 10.0f / wi.speed, 0.1f, 0, 0, qfalse);
					VectorCopy(move.endpos, bestorigin);
					//BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f, frames = %f\n", FloatTime(), VectorLength(dir), dist * 10 / wi.speed);
				}
				//if not that skilled do linear prediction
				else if (aim_skill > 0.4f) {
					VectorSubtract(entinfo.origin, bs->origin, dir);
					//distance towards the enemy
					dist = VectorLength(dir);
					//direction the enemy is moving in
					...
					//BOTPRINT(PRT_MESSAGE, "speed = %f, wi->speed = %f\n", speed, wi->speed);
					//best spot to aim at
		...
		if (aim_skill > 0.6f && wi.proj.damagetype & DAMAGETYPE_RADIAL) {
			...
			if (entinfo.origin[2] < bs->origin[2] + 16.0f) {
				...
				if (trace.startsolid) groundtarget[2] = entinfo.origin[2] - 16.0f;
				else groundtarget[2] = trace.endpos[2] - 8.0f;
					...
					if (VectorLengthSquared(dir) < Square(60.0f)) {
						...
						if (VectorLengthSquared(dir) > Square(100.0f)) {
							...
							if (trace.fraction >= 1.0f) {
								//BOTPRINT(PRT_MESSAGE, "%1.1f aiming at ground\n", AAS_Time());
		...
		bestorigin[0] += 20.0f * crandom() * (1.0f - aim_accuracy);
		bestorigin[1] += 20.0f * crandom() * (1.0f - aim_accuracy);
		bestorigin[2] += 10.0f * crandom() * (1.0f - aim_accuracy);
	}
	else {
		//
	...
	if (wi.number == WP_PISTOL ||
        ...
        wi.number == WP_HIGHCAL ) {
		//distance towards the enemy
		dist = VectorLength(dir);
		if (dist > 150.0f) dist = 150.0f;
		f = 0.6f + dist / 150.0f * 0.4f;
		aim_accuracy *= f;
	}
	//add some random stuff to the aim direction depending on the aim accuracy
	if (aim_accuracy < 0.8f) {
		VectorNormalize(dir);
		for (i = 0; i < 3; i++) dir[i] += 0.3f * crandom() * (1.0f - aim_accuracy);
	}
	//set the ideal view angles
	vectoangles(dir, bs->ideal_viewangles);
	//take the weapon spread into account for lower skilled bots
	bs->ideal_viewangles[PITCH] += 6.0f * wi.vspread * crandom() * (1.0f - aim_accuracy);
	bs->ideal_viewangles[PITCH] = AngleMod(bs->ideal_viewangles[PITCH]);
	bs->ideal_viewangles[YAW] += 6.0f * wi.hspread * crandom() * (1.0f - aim_accuracy);
	bs->ideal_viewangles[YAW] = AngleMod(bs->ideal_viewangles[YAW]);
	//if the bots should be really challenging
	if (bot_challenge.integer) {
		//if the bot is really accurate and has the enemy in view for some time
		if (aim_accuracy > 0.9f && bs->enemysight_time < FloatTime() - 1.0f) {
			//set the view angles directly
			if (bs->ideal_viewangles[PITCH] > 180.0f) bs->ideal_viewangles[PITCH] -= 360.0f;
...
================
BotTryToReload
 - Try to reload our weapon
================
*/
void BotTryToReload(bot_state_t *bs){
	if ( (bs->cur_ps.stats[STAT_AMMO] == 0 && bs->cur_ps.ammo[bs->weaponnum] > 0) || ( bs->weaponnum == WP_SHOTGUN && bs->cur_ps.stats[STAT_AMMO] < 12) ) {
		Cmd_Reload_f( &g_entities[bs->entitynum], qtrue );
	}
...
void BotCheckAttack(bot_state_t *bs) {
	...
	bsp_trace_t trace;
	aas_entityinfo_t entinfo;
	vec3_t mins = {-8.0f, -8.0f, -8.0f}, maxs = {8.0f, 8.0f, 8.0f};

	attackentity = bs->enemy;
	//
	BotEntityInfo(attackentity, &entinfo);
	//
	reactiontime = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0.0f, 1.0f);
	if (bs->enemysight_time > FloatTime() - reactiontime) return;
	if (bs->teleport_time > FloatTime() - reactiontime) return;
	//if changing weapons
	if (bs->weaponchange_time > FloatTime() - 0.1f) return;
	//check fire throttle characteristic
	if (bs->firethrottlewait_time > FloatTime()) return;
	firethrottle = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_FIRETHROTTLE, 0.0f, 1.0f);
	...
	if (bsptrace.fraction < 1.0f && bsptrace.ent != attackentity)
		return;
	...
	VectorMA(start, -12.0f, forward, start);
	...
	if (trace.ent != attackentity || attackentity >= MAX_CLIENTS) {
		...
		if (wi.proj.damagetype & DAMAGETYPE_RADIAL) {
			if (trace.fraction * 1000.0f < wi.proj.radius) {
				points = (wi.proj.damage - 0.5f * trace.fraction * 1000.0f) * 0.5f;
				if (points > 0.0f) {
					return;
				}
			}
...
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	...
	dist = fabsf(movedir[0]) * size[0] + fabsf(movedir[1]) * size[1] + fabsf(movedir[2]) * size[2];
	dist *= 0.5f;
	//
	...
	if (health) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
			...
			for (i = 0; i < 3; i++) {
				if (movedir[i] < 0.0f) dist += fabsf(movedir[i]) * fabsf(bboxmaxs[i]);
				else dist += fabsf(movedir[i]) * fabsf(bboxmins[i]);
			}
			...
			if (i >= 0) {
				...
				VectorSet(activategoal->goal.mins, 8.0f, 8.0f, 8.0f);
				VectorSet(activategoal->goal.maxs, -8.0f, -8.0f, -8.0f);
				//
				for (i = 0; i < 3; i++)
				{
					if (movedir[i] < 0.0f) activategoal->goal.maxs[i] += fabsf(movedir[i]) * fabsf(extramaxs[i]);
					else activategoal->goal.mins[i] += fabsf(movedir[i]) * fabsf(extramins[i]);
				} //end for
				//
...
void BotCheckConsoleMessages(bot_state_t *bs) {
	...
	while((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		...
		if (trap_BotNumConsoleMessages(bs->cs) < 10) {
		    //Skip out of the delay 
			//if it is a chat message the bot needs some time to read it
			if (g_gametype.integer != GT_HUB && m.type == CMS_CHAT && m.time > FloatTime() - (1 + random())) break;
		}
		//
		ptr = m.message;
		...
		if (!BotMatchMessage(bs, m.message)) {
			...
			if (m.type == CMS_CHAT && !bot_nochat.integer) {
				...
				if (bs->ainode != AINode_Stand && BotValidChatPosition(bs) && !TeamPlayIsOn()) {
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	VectorSubtract( bs->origin, state->pos.trBase, dir);
	trap_BotMoveInDirection(bs->ms, dir, 400, MOVE_WALK);
...
==================
*/
void BotCheckForC4(bot_state_t *bs, entityState_t *state) {
	// if this is not a prox mine
	if (state->eType != ET_MISSILE || state->weapon != WP_C4)
		return;
	...
	if (!(bs->inventory[INVENTORY_ASSAULT] > 0 && bs->inventory[INVENTORY_CLIPS] > 0) &&
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		bs->setupcount = 0;
		//
	    if (bs->bottype == BOT_TRADER )
	    {
	    	int i;
	    	char merchant_names[MAX_INFO_STRING];
	    	merchant_names[0]='\0';
	    	for (i=0; i < MAX_CLIENTS; i++) {
	    		if (BotGetType(i) == BOT_MERCHANT) {
					Q_strcat(merchant_names, MAX_INFO_STRING, level.clients[i].pers.netname);
					Q_strcat(merchant_names, MAX_INFO_STRING, " to ");
	    		}
	    	}
	    	//take off the last " to " bit
	    	merchant_names[strlen(merchant_names) - 4] = '\0';
	    	G_Say( g_entities+bs->client, g_entities+bs->client, SAY_TELL, va("%s patrol %s", name, merchant_names));
	    }
	...
	if (!BotIntermission(bs) && !BotIsObserver(bs)) {
		if (g_gametype.integer == GT_HUB && !( bs->bottype == BOT_TRADER || bs->bottype == BOT_INFORMANT || bs->bottype == BOT_MERCHANT ) ) {
			if (BotCanSeePlayer(bs)) {
				if (bs->wait_time < FloatTime()){
					bs->stand_time = FloatTime() + BotChatTime(bs);
					AIEnter_Stand(bs, "seek ltg: chat to player");
				}else {
					AIEnter_Wait(bs, "seek ltg: wait for player");
				}
			}
			
			
			if (bs->wait_time < FloatTime()) {
				AIEnter_Wait(bs, "BotEnterWait");
				bs->wait_time = FloatTime() + 10;
			}
		} else {
			//do team AI
			BotTeamAI(bs);
		...
		if (g_gametype.integer == GT_HUB) {
			if (bs->idle_time < FloatTime()) {
				int idle;
				bs->idle_time = FloatTime() + 10 + (10 * random());
				level.clients[ bs->client ].ps.pm_flags |= PMF_BORED;
				idle = (level.clients[ bs->client ].ps.stats[STAT_IDLE] + 1) % 3;
				level.clients[ bs->client ].ps.stats[STAT_IDLE] = idle;
			}
			if (gs.INPLAY == 0 ) {
