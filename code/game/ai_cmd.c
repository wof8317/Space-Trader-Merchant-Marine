...
int BotGetItemTeamGoal(char *goalname, bot_goal_t *goal) {
	...
	//try and find the location if we can't find an item with that name.
	i = trap_BotGetMapLocationGoal(goalname, goal);
	if (i > 0) {
		goal->areanum = BotPointAreaNum(goal->origin);
		return qtrue;
	}
	//try and find a client with the goalname
	client = FindClientByName(goalname);
	...
	if (entinfo.valid) {
		...
		if (areanum) {// && trap_AAS_AreaReachability(areanum)) {
			...
			goal->entitynum = client;
			VectorSet(goal->mins, -32, -32, -32);
			VectorSet(goal->maxs, 32, 32, 32);
			return qtrue;
		}
	}
	return qfalse;
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match) {
	...
	while(1) {
		...
		//Copy out goal we filled out earlier over the waypoints goal
		memcpy(&newwp->goal, &goal, sizeof ( bot_goal_t ));
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	//you can use boss instead of the actual boss name.
	if ( !Q_stricmpn("boss", teammate, 4)){
		for(i=0; i < MAX_CLIENTS; i++) {
			if (BotGetType( i ) == BOT_BOSS) {
				ClientName(i, teammate, MAX_MESSAGE_SIZE);
				break;
			}
		}
	}
...
void BotMatch_Camp(bot_state_t *bs, bot_match_t *match) {
	...
	else if (trap_BotGetMapLocationGoal(itemname, &goal)) {
		bs->teamgoal.entitynum = goal.entitynum;
		bs->teamgoal.areanum = goal.areanum;
		VectorCopy(goal.origin, bs->teamgoal.origin);
		VectorSet(bs->teamgoal.mins, -8, -8, -8);
		VectorSet(bs->teamgoal.maxs, 8, 8, 8);
	}
...
==================
*/
void BotMatch_FollowPath(bot_state_t *bs, bot_match_t *match) {
    char path_name[MAX_MESSAGE_SIZE];
	if (!TeamPlayIsOn()) return;
	//if not addressed to this bot
	if (!BotAddressedToBot(bs, match)) return;
	//get the patrol waypoints
	trap_BotMatchVariable(match, KEYAREA, path_name, MAX_MESSAGE_SIZE);
	bs->curpatrolpoint = BotFindGlobalWayPoint(path_name);
...
void BotMatch_WhatAreYouDoing(bot_state_t *bs, bot_match_t *match) {
	...
	switch(bs->ltgtype) {
		...
		case LTG_FOLLOWPATH:
		{
		    BotAI_BotInitialChat(bs,"pathing",NULL);
		    break;
		}
		case LTG_GETFLAG:
		{
...
extern qboolean Inv_BeginTrade( int client, int trans_id, const char* merchant );

void BotMatch_Dialog( bot_state_t *bs, bot_match_t *match) {
	char netname[MAX_MESSAGE_SIZE], botname[MAX_MESSAGE_SIZE], message[MAX_MESSAGE_SIZE];
	int client;
	vec3_t dir;
	
	//Check to see of the message was addressed to this bot.
	if (!BotAddressedToBot(bs, match)) return;
	
	//get the netname
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = ClientFromName(netname);
	ClientName(bs->client, botname, MAX_MESSAGE_SIZE);
	trap_BotMatchVariable(match, MESSAGE, message, sizeof(message));
	if (Q_strncmp(botname, message, MAX_MESSAGE_SIZE) == 0) message[0] = 0;
	//Get the vector between the end of the trace and the bots origin
	VectorSubtract(level.clients[client].ps.origin, bs->origin, dir);
	//Convert that vector to the angles to look in that direction
	vectoangles(dir, bs->ideal_viewangles);
	bs->ideal_viewangles[PITCH]=0;
	//
	// get the bot to stand and wait for interaction to be over...
	//
	//dont always chat
	if (bs->lastchat_time < FloatTime())
		G_Voice( &g_entities[bs->entitynum], &g_entities[client], SAY_TELL, "greeting", qfalse);

	//BotAI_BotInitialChat(bs, "greeting", netname, NULL);
	trap_BotEnterChat(bs->cs, client, CHAT_TELL);
	bs->stand_time = FloatTime() + 5.0f;
	bs->lastchat_time = FloatTime() + 3.0f;
	if (bs->bottype != BOT_TRADER) AIEnter_Stand(bs, "seek ltg: chat to player");
	
	// send event to rules engine to handle interaction
	G_ST_exec( ST_BOT_INTERACT, client, bs->client, g_entities[bs->entitynum].client->pers.npc_id, message );
}
...
int BotMatchMessage(bot_state_t *bs, char *message) {
	...
	switch(match.type)
	{
		...
		case MSG_FOLLOWPATH:
		{
		    BotMatch_FollowPath(bs,&match);
		    break;
		}
		case MSG_DIALOG:
		{
			BotMatch_Dialog(bs,&match);
			break;
		}
		//CTF & 1FCTF
