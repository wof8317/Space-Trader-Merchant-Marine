...
void trap_LocateGameData( gentity_t *gEnts, int numGEntities, int sizeofGEntity_t,
						 playerState_t *clients, int sizeofGClient, globalState_t * gs ) {
	syscall( G_LOCATE_GAME_DATA, gEnts, numGEntities, sizeofGEntity_t, clients, sizeofGClient, gs );
}
...
int trap_BotAllocateClient( int player ) {
	return syscall( G_BOT_ALLOCATE_CLIENT, player );
}
...
float trap_Characteristic_Float(int character, int index) {
	fi_t t;
	t.i = syscall( BOTLIB_AI_CHARACTERISTIC_FLOAT, character, index );
	return t.f;
}

float trap_Characteristic_BFloat(int character, int index, float min, float max) {
	fi_t t;
	t.i = syscall( BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index, PASSFLOAT(min), PASSFLOAT(max) );
	return t.f;
}
...
float trap_BotAvoidGoalTime(int goalstate, int number) {
	fi_t t;
	t.i = syscall( BOTLIB_AI_AVOID_GOAL_TIME, goalstate, number );
	return t.f;
}
...
void trap_SQL_LoadDB( const char * filename ) {
	syscall( G_SQL_LOADDB, filename );
}
void trap_SQL_Exec( const char* statement) {
	syscall( G_SQL_EXEC, statement);
}
void trap_SQL_Prepare( const char* src ) {
	syscall( G_SQL_PREPARE, src );
}

void trap_SQL_BindText( int i, const char* text ) {
	syscall( G_SQL_BINDTEXT, i, text );
}

void trap_SQL_BindInt( int i, int v ) {
	syscall( G_SQL_BINDINT, i, v );
}

qboolean trap_SQL_Step() {
	return syscall( G_SQL_STEP );
}

int trap_SQL_ColumnCount() {
	return syscall( G_SQL_COLUMNCOUNT );
}

void trap_SQL_ColumnAsText( char * buffer, int size, int i ) {
	syscall( G_SQL_COLUMNASTEXT, buffer, size, i );
}

int trap_SQL_ColumnAsInt( int i ) {
	return syscall( G_SQL_COLUMNASINT, i );
}

void trap_SQL_ColumnName( char * buffer, int size, int i ) {
	syscall( G_SQL_COLUMNNAME, buffer, size, i );
}

int trap_SQL_Done() {
	return syscall( G_SQL_DONE );
}

int trap_SQL_Select( const char * statement, char * buffer, int size ) {
	return syscall( G_SQL_SELECT, statement, buffer, size );
}
