...
#define	RESPAWN_HEALTH		20
#define	RESPAWN_AMMO		40
#define	RESPAWN_HOLDABLE	60
#define	RESPAWN_MEGAHEALTH	35//120
#define	RESPAWN_POWERUP		120
#define RESPAWN_STASH       -1
...
int Pickup_Stash( gentity_t *ent, gentity_t *other ) {
	// make sure we are not already picking up another stash
	if ( (other->client->ps.externalEvent & EV_ITEM_STASH) != EV_ITEM_STASH && G_ST_exec(ST_PICKUP_STASH, ent, other-g_entities))
	{
		ent->wait = -1;
		return RESPAWN_STASH;
	}
	return 0;
}
...
void Add_Ammo (gentity_t *ent, int weapon, int count)
{
	...
	if ( ent->client->ps.ammo[weapon] > G_MaxTotalAmmo(weapon) ) {
		ent->client->ps.ammo[weapon] = G_MaxTotalAmmo(weapon);
	}
}
...
int Pickup_Armor( gentity_t *ent, gentity_t *other ) {
	...
	other->client->ps.shields += ent->item->quantity;

	upperBound = other->client->ps.stats[STAT_MAX_HEALTH] * 2;

	if ( other->client->ps.shields > upperBound ) {
		other->client->ps.shields = upperBound;
	}
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if ( !(ent->flags & FL_DROPPED_ITEM) && !BG_CanItemBeGrabbed( g_gametype.integer, &ent->s, &other->client->ps ) ) {
		return;
	}
...
//		predict = qfalse;
	...
	case IT_HOLDABLE:
		...
		break;
	case IT_STASH:
		respawn = Pickup_Stash(ent, other);
		predict = qfalse;
		break;
	case IT_MONEYBAG:
		respawn = Pickup_Stash(ent, other);
	...
	if ( ent->item->giType != IT_STASH ) {	// stash sends its own event
...
// PlayerExit_Trigger gets called when the player steps on the exit
