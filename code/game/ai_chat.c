...
char *BotWeaponNameForMeansOfDeath(int mod) {
	switch(mod) {
        ...
        case MOD_HIGHCAL: return "High Caliber Handgun";
        case MOD_C4: return "C4";
...
int BotChat_StartLevel(bot_state_t *bs) {
	...
	//don't chat in teamplay
	if (TeamPlayIsOn()) {
		//No voice taunts when teamplay starts
...
int BotChat_EndLevel(bot_state_t *bs) {
	...
	if (TeamPlayIsOn()) 
	{
		if (BotIsFirstInRankings(bs)) {
			// No voice taunts at the end of levels.
			//trap_EA_Command(bs->client, "vtaunt");
		}
		return qtrue;
	}
...
int BotChat_Death(bot_state_t *bs) {
	...
	else
	{
		...
		if (bs->botdeathtype == MOD_WATER)
			...
			if ( ( bs->botdeathtype == MOD_GAUNTLET ) && random() < 0.5) {
...
int BotChat_Random(bot_state_t *bs) {
	...
	if (TeamPlayIsOn()) {
		//no random taunts throughout the game
		//trap_EA_Command(bs->client, "vtaunt");
		return qfalse;			// don't wait
	}
	//
