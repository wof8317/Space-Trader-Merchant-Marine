...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal) {
	...
	//Following a path
	if (bs->ltgtype == LTG_FOLLOWPATH)
	{
	    //if we don't have a current patrol point set, kill our ltg type
		...
		if (trap_BotTouchingGoal(bs->origin, &bs->curpatrolpoint->goal))
		{
			...
			while ( (path = G_Find( path, FOFS(classname), "info_botpath")) != NULL) {
				if (!Q_stricmp(path->targetname,bs->curpatrolpoint->name)) break;
			}
			if (path){
				vec3_t dir;
				bs->stand_time = FloatTime() + path->wait;
				AIEnter_Stand(bs, "BotGetLongTermGoal: wait for a bit");
				bs->curpatrolpoint = BotFindGlobalWayPoint(path->target);

				VectorSubtract(bs->curpatrolpoint->goal.origin, bs->eye, dir);
				vectoangles(dir, bs->ideal_viewangles);
	...
	if (bs->ltgtype == LTG_PATROL && !retreat) {
		...
		//if the bot touches the current goal
		if (trap_BotTouchingGoal(bs->origin, &bs->curpatrolpoint->goal)) {
			//2 second wait when a patrol ends(trader bot hits a merchant)
		...
		memcpy(goal, &bs->curpatrolpoint->goal, sizeof(bot_goal_t));
		if (goal->areanum == 0 ) goal->areanum = BotPointAreaNum(goal->origin);
		return qtrue;
	}
#ifdef CTF
	if (gametype == GT_CTF) {
...
int AINode_Stand(bot_state_t *bs) {
    ...
    if (bs->lastframe_health > bs->inventory[INVENTORY_HEALTH]) {
        if (BotChat_HitTalking(bs)) {
            bs->standfindenemy_time = FloatTime() + BotChatTime(bs) + 0.1f;
            bs->stand_time = FloatTime() + BotChatTime(bs) + 0.1f;
        }
    }
    ...
    if (BotPlayerInView(bs, 10))
    {
    	bs->stand_time = FloatTime() + 3.0f;
    }
    // when done standing
    if (bs->stand_time < FloatTime()) {
        trap_BotEnterChat(bs->cs, 0, bs->chatto);
        AIEnter_Seek_LTG(bs, "stand: time out");
        // look at spawn angles
        VectorCopy(bs->spawn_viewangles, bs->ideal_viewangles);
        return qfalse;
    }
    //
    return qtrue;
...
int AINode_Wait(bot_state_t *bs) {
    // this is just a dummy node that keeps the bot busy doing nothing
    return qtrue;
}
...
int BotSelectActivateWeapon(bot_state_t *bs) {
	//
	if (bs->inventory[INVENTORY_PISTOL] > 0 && bs->inventory[INVENTORY_BULLETS] > 0)
	...
	else if (bs->inventory[INVENTORY_ASSAULT] > 0 && bs->inventory[INVENTORY_CLIPS] > 0)
		return WEAPONINDEX_ASSAULT;
    else if (bs->inventory[INVENTORY_HIGHCAL] > 0 && bs->inventory[INVENTORY_HIGHCALAMMO] > 0)
        return WEAPONINDEX_HIGHCAL;
	...
	if (bs->blockedbyavoidspot_time > FloatTime() &&
		...
		if (bestmine != -1) {
			...
			if (bs->inventory[INVENTORY_ASSAULT] > 0 && bs->inventory[INVENTORY_CLIPS] > 0)
				moveresult->weapon = WEAPONINDEX_ASSAULT;
			else if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 0)
...
int AINode_Seek_LTG(bot_state_t *bs)
{
	...
	if (bs->bottype == BOT_TRADER && bs->ltgtype != LTG_PATROL ) {
	...
	if (bs->check_time < FloatTime()) {
		...
		if (BotNearbyGoal(bs, bs->tfl, &goal, range)) {
			...
			//BotAI_Print(PRT_MESSAGE, "new nearby goal %s\n", buf);
			//time the bot gets to pick up the nearby goal item
			bs->nbg_time = FloatTime() + 4 + range * 0.01f;
...
int AINode_Battle_Chase(bot_state_t *bs)
{
	...
	if (trap_BotTouchingGoal(bs->origin, &goal)) bs->chase_time = 0.0f;
	//if there's no chase time left
	if (!bs->chase_time || bs->chase_time < FloatTime() - 10.0f) {
		AIEnter_Seek_LTG(bs, "battle chase: time out");
		return qfalse;
	}
	//check for nearby goals periodicly
	if (bs->check_time < FloatTime()) {
		bs->check_time = FloatTime() + 1.0f;
		range = 150.0f;
		//
		if (BotNearbyGoal(bs, bs->tfl, &goal, range)) {
			//the bot gets 5 seconds to pick up the nearby goal item
			bs->nbg_time = FloatTime() + 0.1f * range + 1.0f;
			trap_BotResetLastAvoidReach(bs->ms);
...
==================
*/
void AIEnter_StayHere(bot_state_t *bs, char *s) {
	BotRecordNodeSwitch(bs, "stay here", "", s);
	bs->ainode = AINode_StayHere;
}
...
==================
*/
int AINode_StayHere(bot_state_t *bs) {
    //The only way out of this node is via finding an enemy
	//if the bot's health decreased
	if (bs->lastframe_health > bs->inventory[INVENTORY_HEALTH]) {
		if (BotChat_HitTalking(bs)) {
			bs->standfindenemy_time = FloatTime() + BotChatTime(bs) + 0.1f;
	...
	if (bs->standfindenemy_time < FloatTime()) {
		if (BotFindEnemy(bs, -1)) {
			AIEnter_Battle_Fight(bs, "stay: found enemy");
			return qfalse;
		}
		bs->standfindenemy_time = FloatTime() + 1;
	}
