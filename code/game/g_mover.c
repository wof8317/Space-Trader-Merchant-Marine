...
==================
*/
qboolean G_CheckC4Position( gentity_t *check ) {
	vec3_t		start, end;
	trace_t	tr;
...
==================
*/
qboolean G_TryPushingC4( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		if ( check->s.eType == ET_MISSILE ) {
			...
			if ( !strcmp(check->classname, "c4") ) {
				// if this C4 is attached to this mover try to move it with the pusher
				if ( check->enemy == pusher ) {
					if (!G_TryPushingC4( check, pusher, move, amove )) {
						//explode
						check->s.loopSound = 0;
						G_AddEvent( check, EV_C4_TRIGGER, 0 );
...
void Use_Elevator( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
    ...
    next = G_Find(NULL, FOFS(targetname), other->team);
    //Set the next train target as long as we don't already have one.
    if (next && ! ent->nextTrain && ent->prevTrain != next && ent->s.pos.trDuration + ent->s.pos.trTime < level.time ){
        ent->nextTrain = next;
    } else if ( ent->s.pos.trType == TR_STATIONARY ) {
        Reached_Elevator(ent);
    }
}
/*
================
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	TeleportPlayer(other, origin, angles, qtrue );
}
...
void SP_func_button( gentity_t *ent ) {
    ...
    if (!ent->team){
        if (G_SpawnString("floor","", &floor)){
            ent->team = G_NewString(floor);
        }
    }
...
void Reached_Elevator( gentity_t *ent )
{
	...
	if ( ent->nextTrain )
	{
		...
		// if the path_corner has a speed, use that
		if ( ent->nextTrain->speed )
		{
			speed = ent->nextTrain->speed;
		}
		else
		{
			// otherwise use the train's speed
		...
		// start it going
		SetMoverState( ent, MOVER_1TO2, level.time );
		if ( ent->nextTrain->wait || ent->wait )
		{
			ent->nextthink = level.time + ent->nextTrain->wait * 1000 + ent->wait * 1000;
			ent->think = Think_BeginMoving;
			ent->s.pos.trType = TR_STATIONARY;
		}
		ent->prevTrain = ent->nextTrain;
		ent->nextTrain = NULL;
	}
	else
	{
		if ( ent->prevTrain && ent->prevTrain->team )
		{
			useable = G_Find( NULL, FOFS( targetname ), ent->prevTrain->team );
			if ( useable && useable->moverState == MOVER_POS1 )
			{
				MatchTeam( useable, MOVER_1TO2, level.time + 50 );
			}
			if ( useable && useable->moverState == MOVER_POS2 )
			{
				useable->nextthink = level.time + 1000;
			}
		}
	}
}
...
Move the elevator to the start location
===============
*/
void Think_SetupElevator( gentity_t *ent )
{
	gentity_t * target;
	...
	if ( target )
	{
		...
		VectorCopy( target->r.currentOrigin, ent->pos1 );
		...
		VectorCopy( ent->nextTrain->s.origin, ent->s.origin );
		ent->prevTrain = ent->nextTrain;
		ent->nextTrain = NULL;
		SetMoverState( ent, MOVER_1TO2, level.time );
	}
...
"door" use this entity when we reach the corner
*/
void SP_path_corner( gentity_t *self ) {
    ...
    if (G_SpawnString("door","", &door))
    {
        self->team = G_NewString(door);   
    }
...
/*QUAKED func_elevator (0 .5 .8) ? START_ON TOGGLE BLOCK_STOPS
A elevator is a mover that moves between path_corner target points.
elevator MUST HAVE AN ORIGIN BRUSH.
The elevator spawns at the first target it is pointing at.
"model2"    .md3 model to also draw
"speed"     default 100
"dmg"       default 2
"noise"     looping sound to play when the train is in motion
"target"    starting corner
"wait"      default 1
"color"     constantLight color
"light"     constantLight radius
*/
void SP_func_elevator (gentity_t *self) {
    VectorClear (self->s.angles);
    ...
    if ( !self->target ) {
        G_Printf ("func_elevator without a target at %s\n", vtos(self->r.absmin));
        G_FreeEntity( self );
        return;
    }
    if ( !self->wait ) {
    ...
    // a chance to spawn
    self->nextthink = level.time + FRAMETIME;
    self->think = Think_SetupElevator;
}
...
//	trap_SetBrushModel( ent, ent->model );
	vec3_t scale;
	float allscale;
	G_SpawnVector("modelscale_vec", "1 1 1", scale);
	G_SpawnFloat("modelscale", "1", &allscale);
	InitMover( ent );
	...
	ent->s.angles2[0] *= allscale;
	ent->s.angles2[1] *= allscale;
	ent->s.angles2[2] *= allscale;
