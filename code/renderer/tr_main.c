...
void R_RotateForViewer (void) 
{
	...
	viewerMatrix[12] = -DotProduct( origin, tr.viewParms.or.axis[0] );
	viewerMatrix[13] = -DotProduct( origin, tr.viewParms.or.axis[1] );
	viewerMatrix[14] = -DotProduct( origin, tr.viewParms.or.axis[2] );
	viewerMatrix[15] = 1;
...
static void SetFarClip( void )
{
	...
	// set a very far clip plane
	if ( tr.refdef.rdflags & RDF_NOWORLDMODEL ) {
		tr.viewParms.zFar = 8192;
		return;
	}
...
void R_SetupProjection( void )
{
	...
	tr.viewParms.projectionMatrix[0] = 1.0F / tanf( tr.refdef.fov_x * M_PI / 360.0F );
	tr.viewParms.projectionMatrix[4] = 0;
	tr.viewParms.projectionMatrix[8] = 0;
	tr.viewParms.projectionMatrix[12] = 0;

	tr.viewParms.projectionMatrix[1] = 0;
	tr.viewParms.projectionMatrix[5] = 1.0F / tanf( tr.refdef.fov_y * M_PI / 360.0F );
...
=================
*/
void R_MirrorPoint (vec3_t in, affine_t *surface, affine_t *camera, vec3_t out) {
	int		i;
	vec3_t	local;
	vec3_t	transformed;
	float	d;
...
void R_MirrorVector (vec3_t in, affine_t *surface, affine_t *camera, vec3_t out) {
	int		i;
	float	d;
...
qboolean R_GetPortalOrientations( drawSurf_t *drawSurf, int entityNum, 
							 affine_t *surface, affine_t *camera,
...
=================
*/
drawSurf_t* R_AddDrawSurf( surfaceType_t *surface, shader_t *shader, 
	int fogIndex, int dlightMap )
{
	...
	ret = tr.refdef.drawSurfs + index;

	// the sort data is packed into a single 32 bit value so it can be
	// compared quickly during the qsorting process
	ret->sort = R_ComposeSort( tr.currentEntityNum,
		shader, fogIndex, dlightMap );
	ret->surface = surface;
...
{
	return
		((shader->sortedIndex << QSORT_SHADERNUM_SHIFT) & QSORT_SHADERNUM_MASK)  |
		((entityNum << QSORT_ENTITYNUM_SHIFT) & QSORT_ENTITYNUM_MASK) |
		((fogNum << QSORT_FOGNUM_SHIFT) & QSORT_FOGNUM_MASK) |
		((dlightMap << QSORT_DLIGHT_SHIFT) & QSORT_DLIGHT_MASK);
}

void R_DecomposeSort( uint sort, int *entityNum, shader_t **shader, 
	int *fogNum, int *dlightMap )
{
	*fogNum = (sort & QSORT_FOGNUM_MASK) >> QSORT_FOGNUM_SHIFT;
	*shader = tr.sortedShaders[((sort & QSORT_SHADERNUM_MASK) >> QSORT_SHADERNUM_SHIFT) & (MAX_SHADERS - 1)];
	*entityNum = (sort & QSORT_ENTITYNUM_MASK) >> QSORT_ENTITYNUM_SHIFT;
	*dlightMap = (sort & QSORT_DLIGHT_MASK) >> QSORT_DLIGHT_SHIFT;
}
...
void R_AddEntitySurfaces( void )
{
	...
	for( tr.currentEntityNum = 0; tr.currentEntityNum < tr.refdef.num_entities; tr.currentEntityNum++ )
	{
		...
		case RT_RAIL_RINGS:
			...
			fogNum = R_SpriteFogNum( ent );
			//fallthrough intended

		case RT_SKYCUBE:
			shader = R_GetShaderByHandle( ent->e.customShader );
			R_AddDrawSurf( &entitySurface, shader, fogNum, 0 );
			break;
			
		case RT_ORBIT:
			shader = tr.orbitShader;
			R_AddDrawSurf( &entitySurface, shader, fogNum, 0 );
			break;

		case RT_FLARE:
			{
				srfFlare_t *f = (srfFlare_t*)R_GetCommandMemory( sizeof( srfFlare_t ) );

				Com_Memset( f, 0, sizeof( srfFlare_t ) );
				f->surfaceType = SF_FLARE;
				
				shader = R_GetShaderByHandle( ent->e.customShader );
				R_AddDrawSurf( (surfaceType_t*)f, shader, 0, 0 );
			}
			break;
...
================
*/
void Q_EXTERNAL_CALL RE_DebugPolygon( int color, int numPoints, float *points )
{
	int i;
	stateGroup_t sg;
...
/*
================
R_DebugPolygonOutline
================
*/
void Q_EXTERNAL_CALL RE_DebugPolygonOutline( int color, int numPoints, float *points )
{
	int i;
	stateGroup_t sg;
	// draw wireframe outline
	sg = R_StateBeginGroup();
	GL_State( GLS_POLYMODE_LINE | GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE );
...
void R_DebugGraphics( void )
{
	...
	ri.CM_DrawDebugSurface( RE_DebugPolygon );
}
void R_DebugAAS( void )
 {
	if ( !r_debugAas->integer)
		return;
	...
	R_StateSetCull(backEnd.viewParms.isMirror ? GL_FRONT : GL_BACK );
	ri.AAS_DrawDebugSurface(RE_DebugPolygonOutline );
}
