...
void RB_CalcDeformExpand( deformStage_t *ds )
{
	int i;
	
	float scale = ds->deformationWave.amplitude;

	float * RESTRICT xyz = tess.xyz[0];
	const float * RESTRICT normal = tess.normal[0];

	for( i = 0; i < tess.numVertexes; i++, xyz += 4, normal += 4 )
		VectorMA( xyz, scale, normal, xyz );	
}
...
void DeformText( const char *text ) {
	...
	for ( i = 0 ; i < len ; i++ ) {
		...
		if ( ch != ' ' ) {
			...
			RB_AddQuadStampExt( origin, width, height, color, fcol, frow, fcol + size, frow + size, false );
		}
...
void RB_CalcSpecularAlpha( const shaderStage_t *pStage, unsigned char *alphas )
{
	...
	int i;
	int n = tess.numVertexes;
	float* v = tess.xyz[ 0 ];
	float* normal = tess.normal[ 0 ];
	vec3_t lit_dir;

	float spec_int = 0.3F * pStage->specCtrl[0] + 0.59F * pStage->specCtrl[1] + 0.11F * pStage->specCtrl[2];

	VectorCopy( backEnd.currentEntity->lightDir, lit_dir );
	VectorNormalizeFast( lit_dir );

	for( i = 0, alphas += 3; i < n; i++, v += 4, normal += 4, alphas += 4 )
	{
		vec3_t	vdir;
		vec3_t	h;
		float	ndl, ndh;
		...
		ndl = DotProduct( normal, lit_dir );

		if( ndl > 0 )
		{
			VectorSubtract( backEnd.or.viewOrigin, v, vdir );
			VectorNormalizeFast( vdir );

			VectorAdd( vdir, lit_dir, h );
			VectorNormalizeFast( h );

			ndh = DotProduct( normal, h );

			spec = ndh * ndh;	//^2
			spec *= spec;		//^4
			spec *= spec;		//^8
			spec *= spec;		//^16
			spec *= spec;		//^32 - should be decent...
...
void RB_CalcGlowHaloAlpha( const shaderStage_t *pStage, byte * RESTRICT alpha )
{
	...
	float * RESTRICT v = tess.xyz[0];
	float * RESTRICT n = tess.normal[0];

	float gs = (pStage->constantColor[3] ? pStage->constantColor[3] : backEnd.currentEntity->e.shaderRGBA[3]) * (1.0F / 0xFF);
	float gb = pStage->params[0];
	float gp = pStage->params[1];
...
void RB_CalcDiffuseColor2( const shaderStage_t *pStage, byte *color )
{
	...
	float ls = pStage->params[2];
	float lb = pStage->params[3];

	vec3_t c;
	vec3_t l;

	VectorScale( pStage->constantColor, 1.0F / 0xFF, c );
	VectorCopy( backEnd.currentEntity->lightDir, l );

	for( i = 0; i < tess.numVertexes; i++ )
	{
		vec3_t cl;

		float incoming = DotProduct( n, l ) * ls + lb;
		if( incoming < 0 )
			incoming = 0;
		else if( incoming > 1 )
			incoming = 1;

		VectorScale( c, incoming * 0xFF, cl );
