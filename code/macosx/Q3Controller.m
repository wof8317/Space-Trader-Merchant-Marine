...
- (void)showBanner;
{
    ...
    if (!hasShownBanner) {
        ...
        if (showBanner->integer != 0) {
            ...
            splashPanel = [[NSPanel alloc] initWithContentRect:bannerRect styleMask:(NSTitledWindowMask | NSClosableWindowMask | NSMiniaturizableWindowMask) backing:NSBackingStoreBuffered defer:NO];
...
#ifdef DEMO
	installationPathKey = @"DemoInstallationPath";
#else
...
#ifdef DEMO
		dataPath = [installationPath stringByAppendingPathComponent: @"demo"];
#else
        dataPath = [installationPath stringByAppendingPathComponent: @"st"];
#endif        
