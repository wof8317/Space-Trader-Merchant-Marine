...
void CM_TestBoxInBrush( traceWork_t *tw, cbrush_t *brush ) {
   ...
   if ( tw->sphere.use ) {
		...
		for ( i = 6 ; i < brush->numsides ; i++ ) {
			side = cm.brushsides + brush->firstSide + i;
			plane = cm.planes + side->planeNum;
...
================
*/
void CM_TestInLeaf( traceWork_t *tw, dleaf_t *leaf ) {
	int			k;
	int			brushnum;
	cbrush_t	*b;
	cPatch_t	*patch;
...
================
*/
void CM_TraceThroughLeaf( traceWork_t *tw, dleaf_t *leaf ) {
	int			k;
	int			brushnum;
	cbrush_t	*b;
	cPatch_t	*patch;
