...
void Q_EXTERNAL_CALL VM_VmInfo_f( void );
void Q_EXTERNAL_CALL VM_VmProfile_f( void );
...
*/
void VM_Init( void ) {
	//Set these to 0 to use DLL's, or 2 to use QVM's
...
vm_t *VM_Restart( vm_t *vm ) {
	...
	// validate
	if ( (header->vmMagic != VM_MAGIC && header->vmMagic != VM_MAGIC_VER2)
		|| header->bssLength < 0 
		|| header->dataLength < 0 
		|| header->litLength < 0 
		|| header->codeLength <= 0 ) {
	...
	{
		vm->numJumpTableTargets = header->jtrgLength / 4;
		...
		for( i = 0 ; i < header->jtrgLength ; i += 4 )
			vm->jumpTableTargets[i] = LittleLong( vm->jumpTableTargets[i] );
	}
...
vm_t *VM_Create( const char *module, intptr_t (*systemCalls)(intptr_t *), 
	...
	if( header->vmMagic == VM_MAGIC_VER2 )
	{
		...
		vm->jumpTableTargets = (int*)Hunk_Alloc( header->jtrgLength, h_high );
		Com_Memcpy( vm->jumpTableTargets, (byte*)header + header->dataOffset +
			header->dataLength + header->litLength, header->jtrgLength );
