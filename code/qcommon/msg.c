...
void MSG_WriteDeltaFloat( msg_t *msg, float oldV, float newV ) {
	...
	t.f = newV;
	MSG_WriteBits( msg, 1, 1 );
	MSG_WriteBits( msg, t.i, 32 );
}

float MSG_ReadDeltaFloat( msg_t *msg, float oldV ) {
	if ( MSG_ReadBits( msg, 1 ) ) {
		fi_t t;
		t.i = MSG_ReadBits( msg, 32 );
		return t.f;
	}
	return oldV;
}
...
void MSG_WriteDeltaKeyFloat( msg_t *msg, int key, float oldV, float newV ) {
	...
	MSG_WriteBits( msg, 1, 1 );
	t.f = newV;
	MSG_WriteBits( msg, t.i ^ key, 32 );
}

float MSG_ReadDeltaKeyFloat( msg_t *msg, int key, float oldV ) {
	if ( MSG_ReadBits( msg, 1 ) ) {
		fi_t t;
		t.i = MSG_ReadBits( msg, 32 ) ^ key;
		return t.f;
	}
	return oldV;
}
...
void MSG_WriteDeltaPlayerstate( msg_t *msg, struct playerState_s *from, struct playerState_s *to ) {
	...
	int				ammobits;
	int				gamebits;
	int				gamearraybits[ ((sizeof(globalState_t) / sizeof(int))+31) / 32 ];
	int				numFields;
	int				c;
	const netField_t		*field;
	int				*fromF, *toF;
...
//		pcount[i]++;
	...
	gamebits = 0;
	for (i=0 ; i<lengthof(gamearraybits); i++)
		gamearraybits[ i ] = 0;
	...
	for (i=0 ; i<sizeof(globalState_t)/sizeof(int); i++) {
		...
		if (a[ i ] != b[ i ]) {
			gamearraybits[ i>>5 ] |= (1<<(i&31));
			gamebits |= (1<<(i>>5));
		}
	}

	if (!statsbits && !persistantbits && !ammobits && !gamebits) {
		MSG_WriteBits( msg, 0, 1 );	// no change
		return;
	}
	MSG_WriteBits( msg, 1, 1 );	// changed
	...
	if ( gamebits ) {
		MSG_WriteBits( msg, 1, 1 );	// changed
		MSG_WriteBits( msg, gamebits, 32 );
		for (i=0 ; i<lengthof(gamearraybits) ; i++) {
			if ( gamebits & (1<<i) ) {
				int j;
				int * a = (int*)&to->gs;
				MSG_WriteBits( msg, gamearraybits[i], 32 );
				for (j=0 ; j<32; j++) {
					if ( gamearraybits[i] & (1<<j) )
						MSG_WriteLong( msg, a[ i*32+ j ] );
				}
			}
		}
		...
		if ( MSG_ReadBits( msg, 1 ) ) {
			...
			for  (i=0 ; i<32 ; i++) {
				if (bits & (1<<i) ) {
					int j;
					int * a = (int*)&to->gs;
					int arraybits = MSG_ReadBits (msg, 32 );
					for (j=0 ; j<32; j++) {
						if ( arraybits & (1<<j) )
							a[ i*32+j ] = MSG_ReadLong(msg);
					}
				}
			}
		}
