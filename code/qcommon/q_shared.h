...
#ifdef BUILD_TUNCER
#define USE_DRM
#define USE_ALWAYS_VALID
#else
//#define USE_WEBHOST				// allows paks to be sync'd over http from a web source
#define USE_WEBAUTH				// login and get account from a web host
#define USE_DRM					// stkey file stuff
//#define USE_AUTOPATCH
//#define USE_BOOTWITHNOFILES		// web plugin, uses appdata for base path does not error if no default.cfg
//#define USE_CALLHOME				// reports usage to webauth server
//#define DEMO
#endif

#define	Q3_VERSION		"Compiled on "__DATE__" at "__TIME__
...
#if defined( _MSC_VER ) && !defined( Q_DO_NOT_DISABLE_USEFUL_WARNINGS )
...
#pragma warning( disable : 4514 )		// removed unreferenced inline function
...
#include "platform/stdincludes.h"
#include "platform/push_segs.h"

#include "q_platform.h" //ToDo: split this into platform/
#include "platform/compiler-helpers.h"

#endif

/*
	DO NOT INCLUDE ANY PLATFORM HEADERS BEYOND THIS POINT OR ELSE
*/

/*
	REF_PARAM is used to shut up compiler warnings about unreferenced function
	paramaters. Uncomment the following line to disable the compiler-specific
	version and spew errors. Useful when auditing for all the useless data that
	flies around on the stack.
*/
//#define REF_PARAM


#ifdef _DEBUG
#define COMPILE_ASSERT( x ) typedef void *__tag_compiler_assert##__LINE__[(x) ? 1 : -1];
#else
...
#define offset_ptr( base, T, f ) ((byte*)(base) + offsetof( T, f ))
...
typedef enum {
	...
	EXEC_ONSERVER,		// send this command directly to the server
} cbufExec_t;
...
extern const vec4_t	g_color_table[9];
...
#define COLOR_GRAY		'8'
#define ColorIndex( c )	(((c) - '0') % (sizeof( g_color_table ) / sizeof( g_color_table[0] )))

#define Q_COLOR_ESCAPE	'^'
#define Q_IsColorString(p)	( (p) && (p)[0] == Q_COLOR_ESCAPE && (p)[1] && (((p)[1] >= '0' && (p)[1] < '9') || (p)[1] == '-' || (p)[1] == 'b') )
...
{
#endif
int QDECL BoxOnPlaneSide (vec3_t emins, vec3_t emaxs, struct cplane_s *plane);
...
void Orthogonalize3x3( float m[3][3] );
...
/*
	direction > 0 = slerp through the short angle
	direction < 0 = slerp through the long angle
	direction = 0 = don't correct the inputs
*/
...
NOGLOBALALIAS void Quat_Ident( quat_t o );
...
extern void Q_StrListCat( char *dst, size_t dstLen, const char *item );
extern char * Q_strcpy_ringbuffer( char * buffer, int size, char * first, char * last, const char * s );
...
void	Q_EXTERNAL_CALL_VA Com_Printf( const char *msg, ... );
...
#define GS_VOTENONE			0x00000000
#define GS_VOTEABSTAIN		0x00000001
#define GS_VOTEYES			0x00000002
#define GS_VOTENO			0x00000004


//	this is the global state that is written to in game and propogated to clients
//	** all entries must be multiples of int's as this structure is treated as an
//	array of int's in msg.c
typedef struct
{
	int		COUNTDOWN;
	int		INPLAY;
	...
	struct {
		...
		unsigned int dead_countdown;
		unsigned int boss_spawned;
		unsigned int boss_escaping;
...
typedef struct playerState_s {
    ...
    int         weaponAnim;     // current weapon animation
	...
	globalState_t	gs;	// please move me into snapshot
} playerState_t;
...
#define BUTTON_ATTACK2		32			// melee attack
#define	BUTTON_RELOAD		64			// reload
...
//How many miliseconds do we scale the travel time by.  This makes 1 day = 0.5 seconds
#define TRAVEL_SCALE 50
...
//For float to int and back again conversions
typedef union {
	int i;
	float f;
} fi_t;

//Once we ship, we should get rid of this
void BG_ST_Allsolid(int clientNum);

//
//	fn - Format Number, converts an int into text with commas, and currency support
//
#define FN_NORMAL		(0)
#define FN_DATE			(0)// fixme
#define FN_CURRENCY		(1<<0)		// format with $ and < 0 will be red
#define FN_DISABLED		(1<<1)		// grey out
#define	FN_GOOD			(1<<2)		// high light
#define FN_BAD			(1<<3)		// red
#define FN_TIME			(1<<4)
#define FN_SHORT		(1<<5)		// short numbers i.e. 1.4M
#define FN_RANK			(1<<6)		// 1st 2nd 3rd ...
#define FN_PLAIN		(1<<7)		// no commas
#define FN_PERCENT		(1<<9)		// adds a '%'

extern char* fn( int number, int flags );
extern int fn_buffer( char * buffer, int number, int flags );
