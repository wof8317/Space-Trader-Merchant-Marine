...
void VM_Compile( vm_t *vm, vmHeader_t *header ) {
	...
	buf = Hunk_AllocateTempMemory( maxLength );
	jused = Hunk_AllocateTempMemory( header->instructionCount + 2 );
	...
	{
		int jtt = vm->jumpTableTargets[i];

		if( jtt < header->instructionCount + 2 )
			jused[jtt] = 1;
	}
