...
============
*/
cvar_t* Q_EXTERNAL_CALL Cvar_Get( const char *var_name, const char *var_value, int flags ) {
	cvar_t	*var;
	long	hash;
...
============
*/
void Q_EXTERNAL_CALL Cvar_Toggle_f( void )
{
	int		v;
...
============
*/
void Q_EXTERNAL_CALL Cvar_Set_f( void )
{
	int		i, c, l, len;
...
============
*/
void Q_EXTERNAL_CALL Cvar_SetU_f( void )
{
	cvar_t	*v;
...
============
*/
void Q_EXTERNAL_CALL Cvar_SetS_f( void )
{
	cvar_t	*v;
...
============
*/
void Q_EXTERNAL_CALL Cvar_SetA_f( void )
{
	cvar_t	*v;
...
============
*/
void Q_EXTERNAL_CALL Cvar_Reset_f( void )
{
	if ( Cmd_Argc() != 2 ) {
...
============
*/
void Q_EXTERNAL_CALL Cvar_List_f( void )
{
	cvar_t	*var;
	int		i;
	char	*match;
...
/*
=====================
Cvar_InfoString_Modified
=====================
*/
qboolean Cvar_InfoString_Modified( int bit, char * buffer, int size ) {

	cvar_t	*var;
	int i;

	for (i=0,var = cvar_vars ; var ; var = var->next) {
		if (var->flags & bit && var->modified ) {

			int k = strlen( var->name );
			int v = strlen( var->string );

			if ( i+k+v+2 >= size )
				break;

			buffer[ i++ ] = '\\';
			Q_strncpyz( buffer + i, var->name, size - i );
			i += k;

			buffer[ i++ ] = '\\';
			Q_strncpyz( buffer + i, var->string, size - i );
			i += v;
