...
void SockadrToNetadr (struct sockaddr_in *s, netadr_t *a, netadrtype_t type)
{
...
qboolean Sys_GetPacketData(netadr_t *net_from, msg_t *net_message )
{
	...
	if (net_from->socket == 0)
		return qfalse;
	
	fromlen = sizeof(from);
	ret = recvfrom (net_from->socket, net_message->data, net_message->maxsize
...
int NET_IPSocket (char *net_interface, int port, netadrtype_t type)
{
	...
	if (type == NA_IP)
	{
		...
		{
			Com_Printf ("ERROR: TCP_OpenSocket: socket: %s", NET_ErrorString());
			return 0;
		}
	}
	...
	{
		Com_Printf ("ERROR: OpenSocket: ioctl FIONBIO:%s\n", NET_ErrorString());
		return 0;
	}
	...
	{
		int resp;
		resp = connect(newsocket, &address, sizeof(address));
		if ( !(resp == 0 || (resp == -1 && errno == EINPROGRESS )) )
		{
			Com_Printf ("ERROR: TCP_OpenSocket: connect: %s\n", NET_ErrorString());
			close(newsocket);
			return 0;
		}
	}
