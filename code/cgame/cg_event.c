...
static void CG_ItemPickup( int itemNum ) {
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		if ( cg_autoswitch.integer && bg_itemlist[itemNum].giTag > cg.weaponSelect && bg_itemlist[itemNum].giTag != WP_GRENADE &&
	...
	switch( bg_itemlist[itemNum].giType ) {
		case IT_ARMOR:
			trap_Cmd_ExecuteText( EXEC_NOW, va("st_cp T_Shield_Pickup stash %d",bg_itemlist[itemNum].quantity) );
			break;
		case IT_HEALTH:
			trap_Cmd_ExecuteText( EXEC_NOW, va("st_cp T_Health_Pickup stash %d",bg_itemlist[itemNum].quantity) );
			break;
	}
...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( attacker == cg.snap->ps.clientNum ) {
		...
		if ( cgs.gametype < GT_TEAM ) {
			s = va("You got %s\n%s place with %i", targetName,
				fn( (cg.snap->ps.persistant[PERS_RANK] & ~RANK_TIED_FLAG) + 1, FN_RANK ),
				cg.snap->ps.persistant[PERS_SCORE] );
		} else {
			s = va("%s got you", targetName );
		}
		trap_Cmd_ExecuteText( EXEC_NOW, va("st_cp \"%s\"", s) );
	}
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		case MOD_MACHINEGUN:
			...
			break;
		case MOD_SHOTGUN:
			message = "was shotgunned into a fine mist by";
			break;
		case MOD_GRENADE:
			message = "ate";
			message2 = "'s grenade";
		...
		case MOD_TELEFRAG:
			...
			break;
		case MOD_HIGHCAL:
			message = "thought he was immune to bullets from";
			message2 = "'s Highcal";
			break;
		case MOD_PISTOL:
			message = "almost survived an attack from";
			message2 = "'s Machine Pistol - but didn't";
			break;
		case MOD_ASSAULT:
			message = "gained a few new orifices courtesy of";
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_FOOTSTEP_METAL:
		...
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, 
				cgs.media.footsteps[ FOOTSTEP_METAL ][Q_rand()&3] );
		}
		break;
	case EV_FOOTSPLASH:
		...
		if (cg_footsteps.integer) {
			trap_S_StartSound (NULL, es->number, CHAN_BODY, 
				cgs.media.footsteps[ FOOTSTEP_SPLASH ][Q_rand()&3] );
		}
		break;
	case EV_FOOTWADE:
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
		...
		break;
//No longer being sent by the server.
/*	case EV_JUMP:
		DEBUGNAME("EV_JUMP");
		trap_S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*jump1.ogg" ) );
		break;*/
	...
	case EV_ITEM_STASH:
		DEBUGNAME("EV_ITEM_STASH");
		{
			int commodity_id	= es->eventParm & 0xFF;
			int stash_qty		= es->eventParm >> 8;
			sfxHandle_t			snd;
			int					txt;

			char s[ MAX_SAY_TEXT ];
			
			//events are global, and if it's not us picking up the stash, break out
			if (es->number != cg.snap->ps.clientNum)
				break;
			
			if ( stash_qty == 0 ) {
				snd = cgs.media.stash_full;
				txt = cgs.media.CG_STASH_FULL;
			} else {
				snd = cgs.media.stash_pickup;
				txt = cgs.media.CG_STASH_PICKUP;
			}

			trap_S_StartSound(NULL, es->number, CHAN_AUTO, snd );
			trap_SQL_Run( s, sizeof(s), txt, commodity_id, stash_qty, 0 );
			trap_Cmd_ExecuteText( EXEC_NOW, va("st_cp \"%s\" stash",s) );
		}
		break;
