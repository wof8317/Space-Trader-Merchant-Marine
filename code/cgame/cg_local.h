...
#define	FADE_TIME			200.0f
#define	DAMAGE_DEFLECT_TIME	100
#define	DAMAGE_RETURN_TIME	400
#define DAMAGE_TIME			500.0f
#define	LAND_DEFLECT_TIME	150
#define	LAND_RETURN_TIME	300.0f
...
#define	ATTACKER_HEAD_TIME	5000
...
typedef struct {
	lerpFrame_t		legs, torso, flag, weapon;
	lerpFrame_t		legs2, torso2, flag2, weapon2;
	int				legsTransTime;
	int				torsoTransTime;
	int				weaponTransTime;
	int				painTime;
...
typedef struct weaponInfo_s
{
	...
	qhandle_t       firstpersonModel;

	modelType_t		handModelType;
	modelType_t		weaponModelType;
	modelType_t		flashModelType;
	modelType_t		firstpersonModelType;
	...
	sfxWeaponAnimationSound_t animationSounds[MAX_WEAPON_ANIMATIONS];
} weaponInfo_t;
...
typedef struct {
	...
	int			intermissionStarted;	// don't play voice rewards, because game will end shortly
										// also holds the mission objective status
	...
	char hostageName[MAX_NAME_LENGTH];
	
	char currentPlanetName[MAX_QPATH];

	char scoreboardMessage[MAX_SAY_TEXT];
	char scoreboardTitle[MAX_SAY_TEXT];
} cg_t;
...
typedef struct {
	...
	qhandle_t	viewShieldShader;
	qhandle_t	viewDamagedShader;
	qhandle_t	viewDamagedShader_vert;
	qhandle_t	tracerShader;
	qhandle_t	crosshairShader[NUM_CROSSHAIRS];
	qhandle_t	crosshairTrade;
	qhandle_t	crosshairTalk;
	...
	qhandle_t	shieldExplosionShader;
	...
	sfxHandle_t	brassSound;
	sfxHandle_t	shellSound;
	...
	sfxHandle_t	stash_pickup;
	sfxHandle_t	stash_full;
	
	qhandle_t carModel[3];

	int		CG_FUGITIVE_ESCAPE;
	int		CG_INTERACT_HINT;
	int		CG_EXPLORE_HINT;
	int		CG_STASH_PICKUP;
	int		CG_STASH_FULL;
	int		CG_NOMORECHANCES;
...
#define MAX_PLAYERS cgs.maxplayers
...
extern	vmCvar_t		cg_crosshairColorR;
extern	vmCvar_t		cg_crosshairColorG;
extern	vmCvar_t		cg_crosshairColorB;
extern	vmCvar_t		cg_crosshairColorA;
...
void CG_AddBufferedSound( sfxHandle_t sfx, int time);

void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback, qboolean uiFullScreen );
...
void CG_PositionRotatedEntityOnTag( refEntity_t *entity, qhandle_t entityModel, const refEntity_t *parent, 
...
void CG_BigExplode( vec3_t playerOrigin, qhandle_t shader );
...
// command from the server are only sent to cg, this forwards commands ignored
// by cgame off to ui.
...
qhandle_t	trap_R_GetBaseShader( const char *name );			// get the base shader for a variant shader
...
int			trap_R_GetShaderVariations( qhandle_t baseShader );
qhandle_t	trap_R_GetShaderVariant( qhandle_t baseShader, int varianceIndex );

void		trap_R_MenuBeginSurf( int menuNum );
void		trap_R_MenuEndSurf( void );

// The vidConfig_t will not change during the life of a cgame.
...
const char *CG_ST_getString( int id );
