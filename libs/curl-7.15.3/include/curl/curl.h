 ...
 */
#if (defined(WIN32) || defined(_WIN32)) && !defined(CURL_STATICLIB) && !defined( _XBOX )
...
typedef int ( CURL_CALL *curl_progress_callback)(void *clientp,
                                      double dltotal,
                                      double dlnow,
                                      double ultotal,
                                      double ulnow);
...
typedef size_t ( CURL_CALL *curl_write_callback)(char *buffer,
                                      size_t size,
                                      size_t nitems,
                                      void *outstream);
...
typedef size_t ( CURL_CALL *curl_read_callback)(char *buffer,
                                      size_t size,
                                      size_t nitems,
                                      void *instream);

typedef int ( CURL_CALL *curl_info_callback)(int code, void *infostream);


#ifndef CURL_NO_OLDIES
  /* not used since 7.10.8, will be removed in a future release */
typedef int ( CURL_CALL *curl_passwd_callback)(void *clientp,
                                    const char *prompt,
                                    char *buffer,
                                    int buflen);
#endif
...
typedef curlioerr ( CURL_CALL *curl_ioctl_callback)(CURL *handle,
                                         int cmd,
                                         void *clientp);
 ...
 * callback routines.
 */
typedef void *( CURL_CALL *curl_malloc_callback)(size_t size);
typedef void ( CURL_CALL *curl_free_callback)(void *ptr);
typedef void *( CURL_CALL *curl_realloc_callback)(void *ptr, size_t size);
typedef char *( CURL_CALL *curl_strdup_callback)(const char *str);
typedef void *( CURL_CALL *curl_calloc_callback)(size_t nmemb, size_t size);
...
typedef int ( CURL_CALL *curl_debug_callback)
...
typedef CURLcode ( CURL_CALL *curl_ssl_ctx_callback)(CURL *curl,    /* easy handle */
...
typedef enum {
  ...
  */
  ...
  /* Function taht will be called to for each handle in a curl_multi_info_read
   */
  CINIT(INFOFUNCTION, FUNCTIONPOINT, 142),
  /* Function taht will be called to for each handle in a curl_multi_info_read
   */
  CINIT(INFODATA, OBJECTPOINT, 143),
...
CURL_EXTERN int ( CURL_CALL curl_strequal)(const char *s1, const char *s2);
CURL_EXTERN int ( CURL_CALL curl_strnequal)(const char *s1, const char *s2, size_t n);
 ...
 */
CURL_EXTERN CURLFORMcode CURL_CALL curl_formadd(struct curl_httppost **httppost,
 ...
 */
CURL_EXTERN void CURL_CALL curl_formfree(struct curl_httppost *form);
 ...
 */
CURL_EXTERN char * CURL_CALL curl_getenv(const char *variable);
 ...
 */
CURL_EXTERN char * CURL_CALL curl_version(void);
 ...
 * error occurred.
 */
CURL_EXTERN char * CURL_CALL curl_escape(const char *string, int length);
 ...
 * occurred.
 */
CURL_EXTERN char * CURL_CALL curl_unescape(const char *string, int length);
 ...
 */
CURL_EXTERN void CURL_CALL curl_free(void *p);
 ...
 * uses libcurl
 */
CURL_EXTERN CURLcode CURL_CALL curl_global_init(long flags);
 ...
 */
CURL_EXTERN CURLcode CURL_CALL curl_global_init_mem(long flags,
 ...
 * that uses libcurl
 */
CURL_EXTERN void CURL_CALL curl_global_cleanup(void);
 ...
 */
CURL_EXTERN struct curl_slist * CURL_CALL curl_slist_append(struct curl_slist *,
                                                 const char *);
 ...
 */
CURL_EXTERN void CURL_CALL curl_slist_free_all(struct curl_slist *);
 ...
 */
CURL_EXTERN time_t CURL_CALL curl_getdate(const char *p, const time_t *unused);
...
typedef void (CURL_CALL *curl_lock_function)(CURL *handle,
                                   curl_lock_data data,
                                   curl_lock_access locktype,
                                   void *userptr);
typedef void (CURL_CALL *curl_unlock_function)(CURL *handle,
                                     curl_lock_data data,
                                     void *userptr);
...
CURL_EXTERN CURLSH * CURL_CALL curl_share_init(void);
CURL_EXTERN CURLSHcode CURL_CALL curl_share_setopt(CURLSH *, CURLSHoption option, ...);
CURL_EXTERN CURLSHcode CURL_CALL curl_share_cleanup(CURLSH *);
 ...
 * struct. See above.
 */
CURL_EXTERN curl_version_info_data * CURL_CALL curl_version_info(CURLversion);
 ...
 */
CURL_EXTERN const char * CURL_CALL curl_easy_strerror(CURLcode);
 ...
 * for printing meaningful error messages.
 */
CURL_EXTERN const char * CURL_CALL curl_share_strerror(CURLSHcode);
