...
static int fputc_int( int c, FILE *f ) { return fputc( c, f ); }
...
int curl_mprintf(const char *format, ...)
{
  ...
  retcode = dprintf_formatf(stdout, fputc_int, format, ap_save);
  va_end(ap_save);
  return retcode;
}
...
int curl_mfprintf(FILE *whereto, const char *format, ...)
{
  ...
  va_list ap_save; /* argument pointer */
  va_start(ap_save, format);
  retcode = dprintf_formatf(whereto, fputc_int, format, ap_save);
  va_end(ap_save);
  return retcode;
}
...
{
  return dprintf_formatf(stdout, fputc_int, format, ap_save);
}

int curl_mvfprintf(FILE *whereto, const char *format, va_list ap_save)
{
  return dprintf_formatf(whereto, fputc_int, format, ap_save);
}
