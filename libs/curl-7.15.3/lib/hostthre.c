...
{
	static __declspec( thread ) XNDNS *xdns;
	static __declspec( thread ) struct hostent he;
	static __declspec( thread ) char *addrs[9];
	static __declspec( thread ) char *no_names = 0;
    ...
    err = XNetDnsLookup( name, NULL, &xdns );

	if( err == 0 )
	{
		while( xdns->iStatus == WSAEINPROGRESS )
			;
		...
		if( xdns->iStatus == 0 )
		{
			...
			he.h_aliases = &no_names;

			for( i = 0; i < (int)xdns->cina && i < 9; i++ )
			{
				addrs[i] = (char*)(xdns->aina + i);
			}
