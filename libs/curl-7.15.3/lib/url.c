                     ...
                     va_list param)
{
  ...
  case CURLOPT_READFUNCTION:
    ...
    if(!data->set.fread)
      ...
      data->set.fread = (curl_read_callback)fread;
    break;
  case CURLOPT_INFOFUNCTION:
    /*
     * Info callback
     */
    data->set.info = va_arg(param, curl_info_callback);
    //we have no default here, just keep it set to null if va_arg returns null
    break;
  case CURLOPT_INFODATA:
    /*
     * Info callback
     */
    data->set.info_data = va_arg(param, void *);
    //we have no default here, just keep it set to null if va_arg returns null
    break;
                      ...
                      bool *protocol_done)
{
  ...
  *asyncp = FALSE; /* assume synchronous resolves by default */
  ...
  if(CURLE_OK != code) {
    ...
    /* try to report back to our info callback */
    if (data->set.info)
        data->set.info(code,data->set.info_data);
  }
  return code;
}
    ...
    if((CURLE_SEND_ERROR == result) && conn->bits.reuse) {
      ...
      if((CURLE_OK == result) || (CURLE_SEND_ERROR == result)) {
        ...
        if(CURLE_OK == result) {
          ...
          if (data->set.info)
            data->set.info(result, data->set.info_data);  
        }
      }
    }
  }
