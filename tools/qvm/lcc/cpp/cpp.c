...
#ifdef _MSC_VER
	static const char *typestrs[] = {"warning: ", "error: ", "error: ", ""};
	const char *tstr = (type >= 0 && type <= (sizeof(typestrs) / sizeof(typestrs[0]))) ? typestrs[type] : "";

	//VC friendly error out under VC
	fprintf(stderr, "%s(%d): %s", cursource->filename, cursource->line, tstr);
#else
	fprintf(stderr, "cpp: ");
